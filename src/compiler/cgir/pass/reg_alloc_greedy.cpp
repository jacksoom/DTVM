...
#include "calc_spill_weights.h"
#include "cg_block_frequency_info.h"
#include "cg_dominators.h"
#include "cg_frame_info.h"
#include "cg_loop_info.h"
#include "cg_register_class_info.h"
#include "cg_register_info.h"
#include "cg_spiller.h"
#include "cg_split_kit.h"
#include "cgir/cg_basic_block.h"
#include "cgir/cg_function.h"
#include "cgir/cg_instruction.h"
...
#include "live_range_edit.h"
#include "live_reg_matrix.h"
#include "live_stacks.h"
...
static unsigned NumGlobalSplits = 0;
static unsigned NumLocalSplits = 0;
static unsigned NumEvicted = 0;

static CgSplitEditor::ComplementSpillMode SplitSpillMode =
    CgSplitEditor::SM_Speed;

static unsigned LastChanceRecoloringMaxDepth = 5;

static unsigned LastChanceRecoloringMaxInterference = 8;

static bool ExhaustiveSearch = false;

static bool EnableDeferredSpilling = false;

// FIXME: Find a good default for this flag and remove the flag.
static unsigned CSRFirstTimeCost = 0;
...
#ifndef NDEBUG
const char *const CgRAGreedy::StageName[] = {
...
CgRAGreedy::CgRAGreedy(CgFunction &MF, RegClassFilterFunc F)
    : ContextObject(MF.getContext()), CgRegAllocBase(F) {
  runOnCgFunction(MF);
}

//===----------------------------------------------------------------------===//
//                     CgLiveRangeEdit delegate methods
//===----------------------------------------------------------------------===//

bool CgRAGreedy::LRE_CanEraseVirtReg(Register VirtReg) {
...
void CgRAGreedy::LRE_WillShrinkVirtReg(Register VirtReg) {
  if (!VRM->hasPhys(VirtReg))
    return;
  ...
  CgLiveInterval &LI = LIS->getInterval(VirtReg);
  Matrix->unassign(LI);
  CgRegAllocBase::enqueue(&LI);
}

void CgRAGreedy::LRE_DidCloneVirtReg(Register New, Register Old) {
  ExtraInfo->LRE_DidCloneVirtReg(New, Old);
}

void CgRAGreedy::ExtraRegInfo::LRE_DidCloneVirtReg(Register New, Register Old) {
...
void CgRAGreedy::enqueueImpl(const CgLiveInterval *LI) { enqueue(Queue, LI); }

void CgRAGreedy::enqueue(PQueue &CurQueue, const CgLiveInterval *LI) {
...
const CgLiveInterval *CgRAGreedy::dequeue() { return dequeue(Queue); }

const CgLiveInterval *CgRAGreedy::dequeue(PQueue &CurQueue) {
  if (CurQueue.empty())
    return nullptr;
  CgLiveInterval *LI = &LIS->getInterval(~CurQueue.top().second);
  CurQueue.pop();
  return LI;
}
...
MCRegister CgRAGreedy::tryAssign(const CgLiveInterval &VirtReg,
...
Register CgRegAllocEvictionAdvisor::canReassign(const CgLiveInterval &VirtReg,
  ...
  for (auto I = Order.begin(), E = Order.end(); I != E && !PhysReg; ++I) {
    ...
    for (; Units.isValid(); ++Units) {
      ...
      CgLiveIntervalUnion::Query subQ(VirtReg, Matrix->getLiveUnions()[*Units]);
...
/// returned true.
void CgRAGreedy::evictInterference(const CgLiveInterval &VirtReg,
                                   MCRegister PhysReg,
                                   SmallVectorImpl<Register> &NewVRegs) {
  ...
  for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
    ...
    // against it.
    ArrayRef<const CgLiveInterval *> IVR = Q.interferingVRegs();
    Intfs.append(IVR.begin(), IVR.end());
  }

  // Evict them second. This will invalidate the queries.
  for (const CgLiveInterval *Intf : Intfs) {
...
bool CgRegAllocEvictionAdvisor::isUnusedCalleeSavedReg(
...
Optional<unsigned>
CgRegAllocEvictionAdvisor::getOrderLimit(const CgLiveInterval &VirtReg,
...
bool CgRegAllocEvictionAdvisor::canAllocatePhysReg(unsigned CostPerUseLimit,
                                                   MCRegister PhysReg) const {
...
MCRegister CgRAGreedy::tryEvict(const CgLiveInterval &VirtReg,
                                CgAllocationOrder &Order,
                                SmallVectorImpl<Register> &NewVRegs,
...
/// CgSpillPlacement and return the static cost of this split in Cost, assuming
/// that all preferences in SplitConstraints are met.
/// Return false if there are no bundles with positive bias.
bool CgRAGreedy::addSplitConstraints(CgInterferenceCache::Cursor Intf,
                                     BlockFrequency &Cost) {
  ArrayRef<CgSplitAnalysis::BlockInfo> UseBlocks = SA->getUseBlocks();

  // Reset interference dependent info.
  SplitConstraints.resize(UseBlocks.size());
  BlockFrequency StaticCost = 0;
  for (unsigned I = 0; I != UseBlocks.size(); ++I) {
    const CgSplitAnalysis::BlockInfo &BI = UseBlocks[I];
    CgSpillPlacement::BlockConstraint &BC = SplitConstraints[I];

    BC.Number = BI.MBB->getNumber();
    Intf.moveToBlock(BC.Number);
    BC.Entry =
        BI.LiveIn ? CgSpillPlacement::PrefReg : CgSpillPlacement::DontCare;
    BC.Exit = (BI.LiveOut &&
               !LIS->getInstructionFromIndex(BI.LastInstr)->isImplicitDef())
                  ? CgSpillPlacement::PrefReg
                  : CgSpillPlacement::DontCare;
    ...
    if (BI.LiveIn) {
      if (Intf.first() <= Indexes->getMBBStartIdx(BC.Number)) {
        BC.Entry = CgSpillPlacement::MustSpill;
        ++Ins;
      } else if (Intf.first() < BI.FirstInstr) {
        BC.Entry = CgSpillPlacement::PrefSpill;
        ++Ins;
      ...
      if (((BC.Entry == CgSpillPlacement::MustSpill) ||
           (BC.Entry == CgSpillPlacement::PrefSpill)) &&
          CgSlotIndex::isEarlierInstr(BI.FirstInstr,
    ...
    if (BI.LiveOut) {
      if (Intf.last() >= SA->getLastSplitPoint(BC.Number)) {
        BC.Exit = CgSpillPlacement::MustSpill;
        ++Ins;
      } else if (Intf.last() > BI.LastInstr) {
        BC.Exit = CgSpillPlacement::PrefSpill;
        ++Ins;
...
bool CgRAGreedy::addThroughConstraints(CgInterferenceCache::Cursor Intf,
                                       ArrayRef<unsigned> Blocks) {
  const unsigned GroupSize = 8;
  CgSpillPlacement::BlockConstraint BCS[GroupSize];
  ...
  for (unsigned Number : Blocks) {
    ...
    // Abort if the spill cannot be inserted at the MBB' start
    CgBasicBlock *MBB = MF->getBlockNumbered(Number);
    // auto FirstNonDebugInstr = MBB->getFirstNonDebugInstr();
    auto FirstNonDebugInstr = MBB->begin();
    if (FirstNonDebugInstr != MBB->end() &&
        CgSlotIndex::isEarlierInstr(
            LIS->getInstructionIndex(*FirstNonDebugInstr),
            SA->getFirstSplitPoint(Number)))
      return false;
    // Interference for the live-in value.
    if (Intf.first() <= Indexes->getMBBStartIdx(Number))
      BCS[B].Entry = CgSpillPlacement::MustSpill;
    else
      BCS[B].Entry = CgSpillPlacement::PrefSpill;

    // Interference for the live-out value.
    if (Intf.last() >= SA->getLastSplitPoint(Number))
      BCS[B].Exit = CgSpillPlacement::MustSpill;
    else
      BCS[B].Exit = CgSpillPlacement::PrefSpill;
...
bool CgRAGreedy::growRegion(GlobalSplitCandidate &Cand) {
...
bool CgRAGreedy::calcCompactRegion(GlobalSplitCandidate &Cand) {
...
BlockFrequency CgRAGreedy::calcSpillCost() {
  BlockFrequency Cost = 0;
  ArrayRef<CgSplitAnalysis::BlockInfo> UseBlocks = SA->getUseBlocks();
  for (const CgSplitAnalysis::BlockInfo &BI : UseBlocks) {
...
///
BlockFrequency CgRAGreedy::calcGlobalSplitCost(GlobalSplitCandidate &Cand,
                                               const CgAllocationOrder &Order) {
  ...
  for (unsigned I = 0; I != UseBlocks.size(); ++I) {
    ...
    if (BI.LiveIn)
      Ins += RegIn != (BC.Entry == CgSpillPlacement::PrefReg);
    if (BI.LiveOut)
      Ins += RegOut != (BC.Exit == CgSpillPlacement::PrefReg);
    while (Ins--)
...
/// stack-bound bundles.  The shared SA/SE CgSplitAnalysis and CgSplitEditor
/// objects must be initialized for the current live range, and intervals
/// created for the used candidates.
///
/// @param LREdit    The CgLiveRangeEdit object handling the current split.
/// @param UsedCands List of used GlobalCand entries. Every BundleCand value
///                  must appear in this list.
void CgRAGreedy::splitAroundRegion(CgLiveRangeEdit &LREdit,
  ...
  for (const CgSplitAnalysis::BlockInfo &BI : UseBlocks) {
    ...
    if (!IntvIn && !IntvOut) {
      LLVM_DEBUG(dbgs() << printCgBBReference(*BI.MBB) << " isolated.\n");
  ...
  for (unsigned I = 0, E = LREdit.size(); I != E; ++I) {
    const CgLiveInterval &Reg = LIS->getInterval(LREdit.get(I));
...
MCRegister CgRAGreedy::tryRegionSplit(const CgLiveInterval &VirtReg,
                                      CgAllocationOrder &Order,
                                      SmallVectorImpl<Register> &NewVRegs) {
  if (!WA->shouldRegionSplitForVirtReg(*TRI, *MF, VirtReg))
...
unsigned CgRAGreedy::calculateRegionSplitCost(const CgLiveInterval &VirtReg,
                                              CgAllocationOrder &Order,
                                              BlockFrequency &BestCost,
                                              unsigned &NumCands,
                                              bool IgnoreCSR) {
...
unsigned CgRAGreedy::doRegionSplit(const CgLiveInterval &VirtReg,
                                   unsigned BestCand, bool HasCompact,
                                   SmallVectorImpl<Register> &NewVRegs) {
  SmallVector<unsigned, 8> UsedCands;
  // Prepare split editor.
  CgLiveRangeEdit LREdit(&VirtReg, NewVRegs, *MF, *LIS, VRM, this, &DeadRemats);
...
/// they don't allocate.
unsigned CgRAGreedy::tryBlockSplit(const CgLiveInterval &VirtReg,
                                   CgAllocationOrder &Order,
                                   SmallVectorImpl<Register> &NewVRegs) {
  ...
  // Tell CgLiveDebugVariables about the new ranges.
  // DebugVars->splitRegister(Reg, LREdit.regs(), *LIS);

  // Sort out the new intervals created by splitting. The remainder interval
  // goes straight to spilling, the new local ranges get to stay RS_New.
  for (unsigned I = 0, E = LREdit.size(); I != E; ++I) {
    const CgLiveInterval &LI = LIS->getInterval(LREdit.get(I));
...
static unsigned getNumAllocatableRegsForConstraints(
    const CgInstruction *MI, Register Reg, const TargetRegisterClass *SuperRC,
    const TargetInstrInfo *TII, const TargetRegisterInfo *TRI,
    const CgRegisterClassInfo &RCI) {
...
unsigned CgRAGreedy::tryInstructionSplit(const CgLiveInterval &VirtReg,
                                         CgAllocationOrder &Order,
                                         SmallVectorImpl<Register> &NewVRegs) {
  ...
  // register.
  CgLiveRangeEdit LREdit(&VirtReg, NewVRegs, *MF, *LIS, VRM, this, &DeadRemats);
  SE->reset(LREdit, CgSplitEditor::SM_Size);

  ArrayRef<CgSlotIndex> Uses = SA->getUseSlots();
  if (Uses.size() <= 1)
    return 0;
  ...
  const TargetRegisterClass *SuperRC =
      WA->getLargestLegalSuperClass(*TRI, CurRC, *MF);
  ...
  // the allocation.
  for (const CgSlotIndex Use : Uses) {
    if (const CgInstruction *MI = Indexes->getInstructionFromIndex(Use))
      if (MI->isFullCopy() ||
    ...
    SE->openIntv();
    CgSlotIndex SegStart = SE->enterIntvBefore(Use);
    CgSlotIndex SegStop = SE->leaveIntvAfter(Use);
...
///
void CgRAGreedy::calcGapWeights(MCRegister PhysReg,
                                SmallVectorImpl<float> &GapWeight) {
  assert(SA->getUseBlocks().size() == 1 && "Not a local interval");
  const CgSplitAnalysis::BlockInfo &BI = SA->getUseBlocks().front();
  ArrayRef<CgSlotIndex> Uses = SA->getUseSlots();
  ...
  for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
    if (!Matrix->query(const_cast<CgLiveInterval &>(SA->getParent()), *Units)
             .checkInterference())
      continue;
    ...
    //
    CgLiveIntervalUnion::SegmentIter IntI =
  ...
  for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
    const CgLiveRange &LR = LIS->getRegUnit(*Units);
    CgLiveRange::const_iterator I = LR.find(StartIdx);
    CgLiveRange::const_iterator E = LR.end();
...
/// basic block.
///
unsigned CgRAGreedy::tryLocalSplit(const CgLiveInterval &VirtReg,
                                   CgAllocationOrder &Order,
                                   SmallVectorImpl<Register> &NewVRegs) {
  ...
  if (Matrix->checkRegMaskInterference(VirtReg)) {
    ...
    ArrayRef<CgSlotIndex> RMS =
        LIS->getRegMaskSlotsInBlock(BI.MBB->getNumber());
    ...
    for (unsigned I = 0; I != NumGaps && RI != RE; ++I) {
      ...
      assert(!CgSlotIndex::isEarlierInstr(RMS[RI], Uses[I]));
      if (CgSlotIndex::isEarlierInstr(Uses[I + 1], RMS[RI]))
        continue;
      // Skip a regmask on the same instruction as the last use. It doesn't
      // overlap the live range.
      if (CgSlotIndex::isSameInstr(Uses[I + 1], RMS[RI]) && I + 1 == NumGaps)
        break;
      ...
      // both gaps.
      while (RI != RE && CgSlotIndex::isEarlierInstr(RMS[RI], Uses[I + 1]))
        ++RI;
    }
    LLVM_DEBUG(dbgs() << '\n');
  }
  ...
  uint64_t Freq = MBFI->getEntryFreq();
  ZEN_ASSERT(Freq != 0);
      ...
      if (Legal && MaxGap < huge_valf) {
        ...
        const float EstWeight = normalizeSpillWeight(
            ...
            Uses[SplitBefore].distance(Uses[SplitAfter]) +
                (LiveBefore + LiveAfter) * CgSlotIndex::InstrDist,
            1);
  ...
  SE->openIntv();
  CgSlotIndex SegStart = SE->enterIntvBefore(Uses[BestBefore]);
  CgSlotIndex SegStop = SE->leaveIntvAfter(Uses[BestAfter]);
  SE->useIntv(SegStart, SegStop);
  SmallVector<unsigned, 8> IntvMap;
  SE->finish(&IntvMap);
  // DebugVars->splitRegister(VirtReg.reg(), LREdit.regs(), *LIS);
...
unsigned CgRAGreedy::trySplit(const CgLiveInterval &VirtReg,
                              CgAllocationOrder &Order,
                              SmallVectorImpl<Register> &NewVRegs,
                              const SmallVirtRegSet &FixedRegisters) {
...
static bool hasTiedDef(CgRegisterInfo *MRI, unsigned reg) {
  for (const CgOperand &MO : MRI->def_operands(reg))
    if (MO.isTied())
      return true;
...
/// recolored.
bool CgRAGreedy::mayRecolorAllInterferences(
    MCRegister PhysReg, const CgLiveInterval &VirtReg,
  ...
  for (MCRegUnitIterator Units(PhysReg, TRI); Units.isValid(); ++Units) {
    ...
    for (const CgLiveInterval *Intf : reverse(Q.interferingVRegs())) {
...
/// exists.
unsigned CgRAGreedy::tryLastChanceRecoloring(
    const CgLiveInterval &VirtReg, CgAllocationOrder &Order,
  ...
  for (MCRegister PhysReg : Order) {
    ...
    for (const CgLiveInterval *RC : RecoloringCandidates) {
...
bool CgRAGreedy::tryRecoloringCandidates(PQueue &RecoloringQueue,
                                         SmallVectorImpl<Register> &NewVRegs,
                                         SmallVirtRegSet &FixedRegisters,
                                         RecoloringStack &RecolorStack,
                                         unsigned Depth) {
  while (!RecoloringQueue.empty()) {
    const CgLiveInterval *LI = dequeue(RecoloringQueue);
...
MCRegister CgRAGreedy::selectOrSplit(const CgLiveInterval &VirtReg,
                                     SmallVectorImpl<Register> &NewVRegs) {
  CutOffInfo = CO_None;
...
MCRegister CgRAGreedy::tryAssignCSRFirstTime(
    const CgLiveInterval &VirtReg, CgAllocationOrder &Order, MCRegister PhysReg,
...
void CgRAGreedy::aboutToRemoveInterval(const CgLiveInterval &LI) {
  // Do not keep invalid information around.
  SetOfBrokenHints.remove(&LI);
}

void CgRAGreedy::initializeCSRCost() {
...
void CgRAGreedy::collectHintInfo(Register Reg, HintsInfo &Out) {
  for (const CgInstruction &Instr : MRI->reg_nodbg_instructions(Reg)) {
    if (!Instr.isFullCopy())
      continue;
...
BlockFrequency CgRAGreedy::getBrokenHintFreq(const HintsInfo &List,
                                             MCRegister PhysReg) {
  BlockFrequency Cost = 0;
...
/// and new register.
void CgRAGreedy::tryHintRecoloring(const CgLiveInterval &VirtReg) {
...
void CgRAGreedy::tryHintsRecoloring() {
  for (const CgLiveInterval *LI : SetOfBrokenHints) {
...
MCRegister CgRAGreedy::selectOrSplitImpl(const CgLiveInterval &VirtReg,
                                         SmallVectorImpl<Register> &NewVRegs,
                                         SmallVirtRegSet &FixedRegisters,
                                         RecoloringStack &RecolorStack,
                                         unsigned Depth) {
  ...
  if ((EnableDeferredSpilling ||
    ...
    NamedRegionTimer T("spill", "CgSpiller", TimerGroupName,
                       TimerGroupDescription, TimePassesIsEnabled);
    CgLiveRangeEdit LRE(&VirtReg, NewVRegs, *MF, *LIS, VRM, this, &DeadRemats);
    spiller().spill(LRE);
    ExtraInfo->setStage(NewVRegs.begin(), NewVRegs.end(), RS_Done);

    // Tell CgLiveDebugVariables about the new ranges. Ranges not being covered
    // by the new regs are kept in LDV (still mapping to the old register),
    // until we rewrite spilled locations in LDV at a later stage.
...
#if 0
void CgRAGreedy::RAGreedyStats::report(MachineOptimizationRemarkMissed &R) {
  using namespace ore;
  if (Spills) {
...
CgRAGreedy::RAGreedyStats CgRAGreedy::computeStats(CgBasicBlock &MBB) {
  RAGreedyStats Stats;
#if 0
  ...
  auto isPatchpointInstr = [](const CgInstruction &MI) {
  ...
  for (CgInstruction &MI : MBB) {
    if (MI.isCopy()) {
      CgOperand &Dest = MI.getOperand(0);
      CgOperand &Src = MI.getOperand(1);
    ...
    if (WA->isLoadFromStackSlot(*TII, MI, FI) && MFI.isSpillSlotObjectIndex(FI)) {
      ++Stats.Reloads;
      continue;
    }
    if (WA->isStoreToStackSlot(*TII, MI, FI) && MFI.isSpillSlotObjectIndex(FI)) {
      ++Stats.Spills;
      continue;
    }
...
CgRAGreedy::RAGreedyStats CgRAGreedy::reportStats(CgLoop *L) {
  RAGreedyStats Stats;

#if 0
  // Sum up the spill and reloads in subloops.
  for (CgLoop *SubLoop : *L)
    Stats.add(reportStats(SubLoop));

  for (CgBasicBlock *MBB : L->getBlocks())
...
bool CgRAGreedy::hasVirtRegAlloc() {
...
bool CgRAGreedy::runOnCgFunction(CgFunction &mf) {
  LLVM_DEBUG(dbgs() << "********** GREEDY REGISTER ALLOCATION **********\n"
                    << "********** Function: " << mf.getName() << '\n');

  MF = &mf;
  WA = &(mf.getContext().getLLVMWorkaround());
  ...
  SpillPlacer = mf.SpillPlacer;
  // DebugVars = &getAnalysis<CgLiveDebugVariables>();

  initializeCSRCost();

  RegCosts = COMPILER::getTargetRegisterCosts(TRI);
  RegClassPriorityTrumpsGlobalness = false;
  ...
  ExtraInfo.emplace();
  EvictAdvisor = mf.EvictAdvisor->getAdvisor(mf, *this);

  VRAI = std::make_unique<CgVirtRegAuxInfo>(*MF, *LIS, *VRM, *Loops, *MBFI);
  ...
  SA = newObject<CgSplitAnalysis>(*VRM, *LIS, *Loops);
  SE = newObject<CgSplitEditor>(*SA, *LIS, *VRM, *DomTree, *MBFI, *VRAI);

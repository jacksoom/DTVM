...
class CgLiveRangeCalc : public NonCopyable {
  const CgFunction *MF = nullptr;
  const CgRegisterInfo *MRI = nullptr;
  CgSlotIndexes *Indexes = nullptr;
  CgDominatorTree *DomTree = nullptr;
  CgVNInfo::Allocator *Alloc = nullptr;

  /// LiveOutPair - A value and the block that defined it.  The domtree node
  /// is redundant, it can be computed as:
  /// MDT[Indexes.getMBBFromIndex(VNI->def)].
  using LiveOutPair = std::pair<CgVNInfo *, CgDomTreeNode *>;

  struct MBB2NumberFunctor {
    using argument_type = const CgBasicBlock *;
    unsigned operator()(const CgBasicBlock *MBB) const {
  ...
  /// Map CgLiveRange to sets of blocks (represented by bit vectors) that
  ...
  using EntryInfoMap = DenseMap<CgLiveRange *, std::pair<BitVector, BitVector>>;
  EntryInfoMap EntryInfos;

  /// Map each basic block where a live range is live out to the live-out
  /// value and its defining block.
  ///
  ...
  /// known to be live-in, but the value has not yet been determined.
  struct LiveInBlock {
    ...
    // Cleared when the final value has been determined and LI has been
    // updated.
    CgDomTreeNode *DomNode;

    // Position in block where the live-in range ends, or CgSlotIndex() if
    // the range passes through the block.  When the final value has been
    // determined, the range from the block start to Kill will be added to
    // LI.
    CgSlotIndex Kill;

    // Live-in value filled in by updateSSA once it is known.
    CgVNInfo *Value = nullptr;

    LiveInBlock(CgLiveRange &LR, CgDomTreeNode *node, CgSlotIndex kill)
  ...
  bool isDefOnEntry(CgLiveRange &LR, ArrayRef<CgSlotIndex> Undefs,
  ...
  bool findReachingDefs(CgLiveRange &LR, CgBasicBlock &UseMBB, CgSlotIndex Use,
                        unsigned PhysReg, ArrayRef<CgSlotIndex> Undefs);

  /// updateSSA - Compute the values that will be live in to all requested
  /// blocks in LiveIn.  Create PHI-def values as required to preserve SSA
  /// form.
  ///
  ...
  /// Transfer information from the LiveIn vector to the live ranges and
  /// update the given @p LiveOuts.
  void updateFromLiveIns();

protected:
  /// Some getters to expose in a read-only way some private fields to
  /// subclasses.
  const CgFunction *getCgFunction() { return MF; }
  const CgRegisterInfo *getRegInfo() const { return MRI; }
  CgSlotIndexes *getIndexes() { return Indexes; }
  CgDominatorTree *getDomTree() { return DomTree; }
  CgVNInfo::Allocator *getVNAlloc() { return Alloc; }

  /// Reset Map and Seen fields.
  void resetLiveOutMap();

public:
  CgLiveRangeCalc() = default;
  ...
  void reset(const CgFunction *mf, CgSlotIndexes *SI, CgDominatorTree *MDT,
             CgVNInfo::Allocator *VNIA);
  ...
  ///
  /// The existing values in @p LR must be live so they jointly dominate @p
  /// Use. If @p Use is not dominated by a single existing value, PHI-defs are
  /// inserted as required to preserve SSA form.
  ///
  /// PhysReg, when set, is used to verify live-in lists on basic blocks.
  void extend(CgLiveRange &LR, CgSlotIndex Use, unsigned PhysReg,
  ...
  // unknown.
  //
  // After calling reset(), add known live-out values and known live-in
  // blocks. Then call calculateValues() to compute the actual value that is
  ...
  /// addLiveInBlock().
  void setLiveOutValue(CgBasicBlock *MBB, CgVNInfo *VNI) {
    Seen.set(MBB->getNumber());
    Map[MBB] = LiveOutPair(VNI, nullptr);
  }

  /// addLiveInBlock - Add a block with an unknown live-in value.  This
  /// function can only be called once per basic block.  Once the live-in
  ...
  ///                live-through, set Kill = CgSlotIndex() and also call
  ///                setLiveOutValue(MBB, 0).
  void addLiveInBlock(CgLiveRange &LR, CgDomTreeNode *DomNode,
                      CgSlotIndex Kill = CgSlotIndex()) {
  ...
  static bool isJointlyDominated(const CgBasicBlock *MBB,

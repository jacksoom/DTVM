...
public:
  ...
  /// Data about a loop.
  ///
  /// Contains the data necessary to represent a loop as a pseudo-node once
  /// it's packaged.
  struct LoopData {
  ...
  struct WorkingData {
    ...
    ///
    /// Get the node currently representing Node, which could be a
    /// containing loop.
    ///
    /// This function should only be called when distributing mass.  As long
    /// as there are no irreducible edges to Node, then it will have
    /// complexity O(1) in this context.
    ///
    /// In general, the complexity is O(L), where L is the number of loop
    /// headers Node has been packaged into.  Since this method is called in
    /// the context of distributing mass, L will be the number of loop
    /// headers an early exit edge jumps out of.
  ...
  ///
  /// Adds all edges from LocalLoopHead to Dist.  Calls addToDist() to add
  /// each successor edge.
  ///
  ...
  ///
  /// Adds an edge to Succ to Dist.  If \c LoopHead.isValid(), then whether
  /// the edge is local/exit/backedge is in the context of LoopHead.
  /// Otherwise, every edge should be a local edge (since all the loops are
  /// packaged up).
  ///
  ...
  ///
  /// Distributes the mass in Source according to Dist.  If
  /// LoopHead.isValid(), backedges and exits are stored in its entry in
  /// Loops.
  ///
...
template <class BT> class BlockFrequencyInfoImpl : BlockFrequencyInfoImplBase {
  ...
  ///
  /// Build up \a Loops using \a LoopInfo.  \a LoopInfo gives us a mapping
  /// from each block to the deepest loop it's in, but we need the inverse.
  /// For each loop, we store in reverse post-order its "immediate" members,
  /// defined as the header, the headers of immediate sub-loops, and all other
  /// blocks in the loop that are not in sub-loops.
  void initializeLoops();

  /// Propagate to a block's successors.
  ///
  /// In the context of distributing mass through \c OuterLoop, divide the
  /// mass currently assigned to \c Node between its successors.
  ///
  /// \return \c true unless there's an irreducible backedge.
  ...
  /// reverse post-order, distribute mass to its successors.  Only visits
  /// nodes that have not been packaged into sub-loops.
  ///
  ...
  ///
  /// Find the irreducible SCCs in \c OuterLoop, add them to \a Loops (in
  /// front of \c Insert), and call \a computeMassInLoop() on each of them.
  ///
  ...
  /// contain a irreducible sub-SCCs.  Use \a computeIrreducibleMass() and
  /// then re-enter \a computeMassInLoop().
  ///
  ...
  /// not handle correctly control-flow graphs containing irreducible loops.
  /// To resolve the problem, we apply a post-processing step, which
  /// iteratively updates block frequencies based on the frequencies of their
  /// predesessors. This corresponds to finding the stationary point of the
  /// Markov chain by an iterative method aka "PageRank computation". The
  /// algorithm takes at most O(|E| * IterativeBFIMaxIterations) steps but
  /// typically converges faster.
  ///
  /// Decide whether we want to apply iterative inference for a given
  /// function.
  bool needIterativeInference() const;

  /// Apply an iterative post-processing to infer correct counts for irr
  /// loops.
  void applyIterativeInference();

  using ProbMatrixType = std::vector<std::vector<std::pair<size_t, Scaled64>>>;

  /// Run iterative inference for a probability matrix and initial
  /// frequencies.
  ...
  /// and backward reachable from exists along edges with positive
  /// probability.
...
public:
  ...
  void forgetBlock(const BlockT *BB) {
    ...
    // avoid invalidating indices. Doing so would have saved some memory,
  ...
  ///
  /// Blocks are printed in the natural iteration order of the function,
  /// rather than reverse post-order.  This provides two advantages:  writing
  /// -analyze tests is easier (since blocks come out in source order), and
  /// even unreachable blocks are printed.
  ///
...
/// Dummy implementation since MachineBasicBlocks aren't Values, so ValueHandles
/// don't apply to them.
template <class BFIImplT> class BFICallbackVH<CgBasicBlock, BFIImplT> {
public:
  BFICallbackVH() = default;
  BFICallbackVH(const CgBasicBlock *, BFIImplT *) {}
};
...
void BlockFrequencyInfoImpl<BT>::calculate(const FunctionT &F,
  ...
  // Visit loops in post-order to find the local mass distribution, and then
  // do the full function.
  computeMassInLoops();
  ...
  if (CheckBFIUnknownBlockQueries) {
    ...
    // blocks, if any. This is to distinguish between known/existing
    // unreachable blocks and unknown blocks.
...
void BlockFrequencyInfoImpl<BT>::setBlockFreq(const BlockT *BB, uint64_t Freq) {
  if (Nodes.count(BB))
    ...
    // If BB is a newly added block after BFI is done, we need to create a
    // new BlockNode for it assigned with a new index. The index can be
    // determined by the size of Freqs.
...
template <class BT> void BlockFrequencyInfoImpl<BT>::initializeLoops() {
  ...
  // Visit nodes in reverse post-order and add them to their deepest
  // containing loop.
  for (size_t Index = 0; Index < RPOT.size(); ++Index) {
...
bool BlockFrequencyInfoImpl<BT>::computeMassInLoop(LoopData &Loop) {
  ...
  if (Loop.isIrreducible()) {
    ...
    for (uint32_t H = 0; H < Loop.NumHeaders; ++H) {
      ...
      // Optional<uint64_t> HeaderWeight =
      // Block->getIrrLoopHeaderWeight();
      Optional<uint64_t> HeaderWeight = 0;
      if (!HeaderWeight) {
    ...
    // using a weight that's in the general range of the other headers'
    // weights, and the minimum seems to perform better than the average.)
...
template <class BT> void BlockFrequencyInfoImpl<BT>::applyIterativeInference() {
  ...
  // The map is used to to index successors/predecessors of reachable blocks
  // in the ReachableBlocks vector
...
void BlockFrequencyInfoImpl<BT>::iterativeInference(
  ...
  while (It++ < MaxIterations && !ActiveSet.empty()) {
    ...
    // Compute a new frequency for the block: NewFreq := Freq \times
    // ProbMatrix. A special care is taken for self-edges that needs to be
    // scaled by (1.0 - SelfProb), where SelfProb is the sum of

...
static bool isMoveInstr(const TargetRegisterInfo &tri, const CgInstruction *MI,
                        CgRegister &Src, CgRegister &Dst, unsigned &SrcSub,
                        unsigned &DstSub) {
  if (MI->isCopy()) {
...
static bool isSplitEdge(const CgBasicBlock *MBB) {
...
bool CgCoalescerPair::setRegisters(const CgInstruction *MI) {
  SrcReg = DstReg = CgRegister();
  SrcIdx = DstIdx = 0;
  NewRC = nullptr;
  ...
  if (CgRegister::isPhysicalRegister(Src)) {
    if (CgRegister::isPhysicalRegister(Dst))
      return false;
    std::swap(Src, Dst);
  ...
  const CgRegisterInfo &MRI = MI->getMF()->getRegInfo();
  ...
  assert(CgRegister::isVirtualRegister(Src) && "Src must be virtual");
  assert(!(CgRegister::isPhysicalRegister(Dst) && DstSub) &&
...
bool CgCoalescerPair::flip() {
  if (CgRegister::isPhysicalRegister(DstReg))
    return false;
...
bool CgCoalescerPair::isCoalescable(const CgInstruction *MI) const {
  if (!MI)
    return false;
  CgRegister Src, Dst;
  unsigned SrcSub = 0, DstSub = 0;
  ...
  if (DstReg.isPhysical()) {
    ...
    return CgRegister(TRI.getSubReg(DstReg, SrcSub)) == Dst;
  } else {
    // DstReg is virtual.
    if (DstReg != Dst)
      return false;
...
/*
void
CgRegisterCoalescer::getAnalysisUsage(AnalysisUsage &AU) const
{
    AU.setPreservesCFG();
    AU.addRequired<AAResultsWrapperPass>();
    AU.addRequired<CgLiveIntervals>();
    AU.addPreserved<CgLiveIntervals>();
    AU.addPreserved<CgSlotIndexes>();
    AU.addRequired<CgLoopInfo>();
    AU.addPreserved<CgLoopInfo>();
    AU.addPreservedID(MachineDominatorsID);
    MachineFunctionPass::getAnalysisUsage(AU);
}
 */

void CgRegisterCoalescer::eliminateDeadDefs() {
  SmallVector<Register, 8> NewRegs;
  CgLiveRangeEdit(nullptr, NewRegs, *MF, *LIS, nullptr, this)
      .eliminateDeadDefs(DeadDefs);
}

bool CgRegisterCoalescer::allUsesAvailableAt(const CgInstruction *OrigMI,
                                             CgSlotIndex OrigIdx,
                                             CgSlotIndex UseIdx) {
  SmallVector<CgRegister, 8> NewRegs;
  return CgLiveRangeEdit(nullptr, NewRegs, *MF, *LIS, nullptr, this)
      .allUsesAvailableAt(OrigMI, OrigIdx, UseIdx);
}

void CgRegisterCoalescer::LRE_WillEraseInstruction(CgInstruction *MI) {
  // MI may be in WorkList. Make sure we don't visit it.
  ErasedInstrs.insert(MI);
}

bool CgRegisterCoalescer::adjustCopiesBackFrom(const CgCoalescerPair &CP,
  ...
  CgSlotIndex CopyIdx = LIS->getInstructionIndex(*CopyMI).getRegSlot();
  ...
  // merge these two pieces of B into a single value number, eliminating a
  // copy. For example:
  //
  //  A3 = B0
  //    ...
  ...
  // the example above.
  CgLiveInterval::iterator BS = IntB.FindSegmentContaining(CopyIdx);
  if (BS == IntB.end())
    return false;
  CgVNInfo *BValNo = BS->valno;

  // Get the location that B is defined at.  Two options: either this value
  // has an unknown definition point or it is defined at CopyIdx.  If unknown,
  // we can't process it.
  if (BValNo->def != CopyIdx)
    return false;

  // AValNo is the value number in A that defines the copy, A3 in the example.
  CgSlotIndex CopyUseIdx = CopyIdx.getRegSlot(true);
  CgLiveInterval::iterator AS = IntA.FindSegmentContaining(CopyUseIdx);
  // The live segment might not exist after fun with physreg coalescing.
  if (AS == IntA.end())
    return false;
  CgVNInfo *AValNo = AS->valno;

  // If AValNo is defined as a copy from IntB, we can potentially process
  // this. Get the instruction that defines this value number.
  CgInstruction *ACopyMI = LIS->getInstructionFromIndex(AValNo->def);
  ...
  // live-range starts.  If there are no intervening live segments between
  // them in IntB, we can merge them.
  if (ValS + 1 != BS)
    return false;

  LLVM_DEBUG(dbgs() << "Extending: " << printReg(IntB.reg(), TRI));

  CgSlotIndex FillerStart = ValS->end, FillerEnd = BS->start;
  ...
  // two value numbers.
  IntB.addSegment(CgLiveInterval::Segment(FillerStart, FillerEnd, BValNo));
  ...
  for (CgLiveInterval::SubRange &S : IntB.subranges()) {
    // Check for SubRange Segments of the form [1234r,1234d:0) which can be
    // removed to prevent creating bogus SubRange Segments.
    CgLiveInterval::iterator SS = S.FindSegmentContaining(CopyIdx);
    if (SS != S.end() && CgSlotIndex::isSameInstr(SS->start, SS->end)) {
    ...
    CgVNInfo *SubBValNo = S.getVNInfoAt(CopyIdx);
    S.addSegment(CgLiveInterval::Segment(FillerStart, FillerEnd, SubBValNo));
    CgVNInfo *SubValSNo = S.getVNInfoAt(AValNo->def.getPrevSlot());
  ...
  if (!RecomputeLiveRange) {
    for (CgLiveInterval::SubRange &S : IntA.subranges()) {
      CgLiveInterval::iterator SS = S.FindSegmentContaining(CopyUseIdx);
...
bool CgRegisterCoalescer::hasOtherReachingDefs(CgLiveInterval &IntA,
                                               CgLiveInterval &IntB,
                                               CgVNInfo *AValNo,
                                               CgVNInfo *BValNo) {
  ...
  for (CgLiveRange::Segment &ASeg : IntA.segments) {
    if (ASeg.valno != AValNo)
      continue;
    CgLiveInterval::iterator BI = llvm::upper_bound(IntB, ASeg.start);
    if (BI != IntB.begin())
      --BI;
...
static std::pair<bool, bool> addSegmentsWithValNo(CgLiveRange &Dst,
                                                  CgVNInfo *DstValNo,
                                                  const CgLiveRange &Src,
                                                  const CgVNInfo *SrcValNo) {
  bool Changed = false;
  bool MergedWithDead = false;
  for (const CgLiveRange::Segment &S : Src.segments) {
    if (S.valno != SrcValNo)
      continue;
    ...
    CgLiveRange::Segment Added = CgLiveRange::Segment(S.start, S.end, DstValNo);
    CgLiveRange::Segment &Merged = *Dst.addSegment(Added);
...
std::pair<bool, bool>
CgRegisterCoalescer::removeCopyByCommutingDef(const CgCoalescerPair &CP,
                                              CgInstruction *CopyMI) {
  CgLiveInterval &IntA =
      LIS->getInterval(CP.isFlipped() ? CP.getDstReg() : CP.getSrcReg());
  CgLiveInterval &IntB =
  ...
  // the example above.
  CgSlotIndex CopyIdx = LIS->getInstructionIndex(*CopyMI).getRegSlot();
  CgVNInfo *BValNo = IntB.getVNInfoAt(CopyIdx);
  assert(BValNo != nullptr && BValNo->def == CopyIdx);

  // AValNo is the value number in A that defines the copy, A3 in the example.
  CgVNInfo *AValNo = IntA.getVNInfoAt(CopyIdx.getRegSlot(true));
  assert(AValNo && !AValNo->isUnused() && "COPY source not live");
  if (AValNo->isPHIDef())
    return {false, false};
  CgInstruction *DefMI = LIS->getInstructionFromIndex(AValNo->def);
  if (!DefMI)
    return {false, false};
  ...
  if (!_llvm_workaround->findCommutedOpIndices(*TII, *DefMI, UseOpIdx,
                                               NewDstIdx))
    return {false, false};

  CgOperand &NewDstMO = DefMI->getOperand(NewDstIdx);
  ...
  // It's not possible to determine whether it's safe to perform the
  // coalescing.
  for (CgOperand &MO : MRI->use_nodbg_operands(IntA.reg())) {
    CgInstruction *UseMI = MO.getParent();
    unsigned OpNo = &MO - &UseMI->getOperand(0);
    CgSlotIndex UseIdx = LIS->getInstructionIndex(*UseMI);
    CgLiveInterval::iterator US = IntA.FindSegmentContaining(UseIdx);
  ...
  CgBasicBlock *MBB = DefMI->getParent();
  CgInstruction *NewMI = _llvm_workaround->commuteInstruction(
      *TII, *DefMI, false, UseOpIdx, NewDstIdx);
  if (!NewMI)
    return {false, false};
  if (CgRegister::isVirtualRegister(IntA.reg()) &&
      Register::isVirtualRegister(IntB.reg()) &&
      !MRI->constrainRegClass(IntB.reg(), MRI->getRegClass(IntA.reg())))
    return {false, false};
  if (NewMI != DefMI) {
    LIS->ReplaceMachineInstrInMaps(*DefMI, *NewMI);
    CgBasicBlock::iterator Pos = CgBasicBlock::iterator(DefMI);
  ...
  for (CgOperand &UseMO :
    ...
    if (UseMO.isUndef())
      continue;
    CgInstruction *UseMI = UseMO.getParent();
    if (UseMI->isDebugInstr()) {
      // FIXME These don't have an instruction index.  Not clear we have
      // enough info to decide whether to do this replacement or not.  For
      // now do it.
      UseMO.setReg(NewReg);
      continue;
    }
    CgSlotIndex UseIdx = LIS->getInstructionIndex(*UseMI).getRegSlot(true);
    CgLiveInterval::iterator US = IntA.FindSegmentContaining(UseIdx);
    ...
    // into BValNo.
    CgSlotIndex DefIdx = UseIdx.getRegSlot();
    CgVNInfo *DVNI = IntB.getVNInfoAt(DefIdx);
    if (!DVNI)
      continue;
    ...
    for (CgLiveInterval::SubRange &S : IntB.subranges()) {
      CgVNInfo *SubDVNI = S.getVNInfoAt(DefIdx);
      if (!SubDVNI)
        continue;
      CgVNInfo *SubBValNo = S.getVNInfoAt(CopyIdx);
  ...
  if (IntA.hasSubRanges() || IntB.hasSubRanges()) {
    ...
    CgSlotIndex AIdx = CopyIdx.getRegSlot(true);
    LaneBitmask MaskA;
    const CgSlotIndexes &Indexes = *LIS->getSlotIndexes();
    for (CgLiveInterval::SubRange &SA : IntA.subranges()) {
      CgVNInfo *ASubValNo = SA.getVNInfoAt(AIdx);
      ...
      IntB.refineSubRanges(
          ...
          [&Allocator, &SA, CopyIdx, ASubValNo,
           &ShrinkB](CgLiveInterval::SubRange &SR) {
            CgVNInfo *BSubValNo = SR.empty()
    ...
    for (CgLiveInterval::SubRange &SB : IntB.subranges()) {
      if ((SB.LaneMask & MaskA).any())
        continue;
      if (CgLiveRange::Segment *S = SB.getSegmentContaining(CopyIdx))
...
bool CgRegisterCoalescer::removePartialRedundancy(const CgCoalescerPair &CP,
                                                  CgInstruction &CopyMI) {
  assert(!CP.isPhys());
  if (!CopyMI.isFullCopy())
    return false;

  CgBasicBlock &MBB = *CopyMI.getParent();

#if 0
    // If this block is the target of an invoke/inlineasm_br, moving the copy
    // into the predecessor is tricker, and we don't handle it.
  ...
  CgSlotIndex CopyIdx = LIS->getInstructionIndex(CopyMI).getRegSlot(true);
  CgVNInfo *AValNo = IntA.getVNInfoAt(CopyIdx);
  assert(AValNo && !AValNo->isUnused() && "COPY source not live");
  if (!AValNo->isPHIDef())
    return false;
  ...
  CgBasicBlock *CopyLeftBB = nullptr;
  for (CgBasicBlock *Pred : MBB.predecessors()) {
    CgVNInfo *PVal = IntA.getVNInfoBefore(LIS->getMBBEndIdx(Pred));
    CgInstruction *DefMI = LIS->getInstructionFromIndex(PVal->def);
    ...
    // Pred, we need to keep the copy of B = A at the end of Pred if we
    // remove B = A from MBB.
  ...
  // updated. If CopyLeftBB is not nullptr, move CopyMI from MBB to CopyLeftBB
  // and update IntA/IntB.
  //
  ...
  if (CopyLeftBB) {
    ...
    // Make sure that B isn't referenced in the terminators (if any) at the
    // end of the predecessor since we're about to insert a new definition
    // of B before them.
    if (InsPos != CopyLeftBB->end()) {
      CgSlotIndex InsPosIdx =
          LIS->getInstructionIndex(*InsPos).getRegSlot(true);
      if (IntB.overlaps(InsPosIdx, LIS->getMBBEndIdx(CopyLeftBB)))
        return false;
    }
#if 0
        LLVM_DEBUG(dbgs() << "\tremovePartialRedundancy: Move the copy to "
                          << printCgBBReference(*CopyLeftBB) << '\t' << CopyMI);
#endif

    // Insert new copy to CopyLeftBB.
    SmallVector<CgOperand, 2> Operands{
        CgOperand::createRegOperand(IntA.reg(), false),
        CgOperand::createRegOperand(IntB.reg(), false)};
    CgInstruction *NewCopyMI = MF->createCgInstruction(
        *CopyLeftBB, InsPos, TII->get(TargetOpcode::COPY), Operands);
    CgSlotIndex NewCopyIdx =
        LIS->InsertMachineInstrInMaps(*NewCopyMI).getRegSlot();
    IntB.createDeadDef(NewCopyIdx, LIS->getVNInfoAllocator());
    for (CgLiveInterval::SubRange &SR : IntB.subranges())
      SR.createDeadDef(NewCopyIdx, LIS->getVNInfoAllocator());

    // If the newly created Instruction has an address of an instruction
    // that was deleted before (object recycled by the allocator) it needs
    // to be removed from the deleted list.
    ErasedInstrs.erase(NewCopyMI);
  }
#if 0
    else {
        LLVM_DEBUG(dbgs() << "\tremovePartialRedundancy: Remove the copy from "
                          << printCgBBReference(MBB) << '\t' << CopyMI);
    }
#endif
  ...
  // Update the liveness.
  SmallVector<CgSlotIndex, 8> EndPoints;
  CgVNInfo *BValNo = IntB.Query(CopyIdx).valueOutOrDead();
  LIS->pruneValue(*static_cast<CgLiveRange *>(&IntB), CopyIdx.getRegSlot(),
                  &EndPoints);
  BValNo->markUnused();
  ...
  for (CgLiveInterval::SubRange &SR : IntB.subranges()) {
    EndPoints.clear();
    CgVNInfo *BValNo = SR.Query(CopyIdx).valueOutOrDead();
    assert(BValNo && "All sublanes should be live");
    LIS->pruneValue(SR, CopyIdx.getRegSlot(), &EndPoints);
    BValNo->markUnused();
    // We can have a situation where the result of the original copy is
    // live, but is immediately dead in this subrange, e.g. [336r,336d:0).
    // That makes the copy appear as an endpoint from pruneValue(), but we
    // don't want it to because the copy has been removed.  We can go ahead
    // and remove that endpoint; there is no other situation here that there
    // could be a use at the same place as we know that the copy is a full
    // copy.
    for (unsigned I = 0; I != EndPoints.size();) {
      if (CgSlotIndex::isSameInstr(EndPoints[I], CopyIdx)) {
...
static bool definesFullReg(const CgInstruction &MI, CgRegister Reg) {
  ...
  for (const CgOperand &Op : MI.operands()) {
    if (!Op.isReg() || !Op.isDef() || Op.getReg() != Reg)
      continue;
    // Return true if we define the full register or don't care about the
    // value inside other subregisters.
...
bool CgRegisterCoalescer::reMaterializeTrivialDef(const CgCoalescerPair &CP,
                                                  CgInstruction *CopyMI,
                                                  bool &IsDefCopy) {
  return false; // TODO
  IsDefCopy = false;
  CgRegister SrcReg = CP.isFlipped() ? CP.getDstReg() : CP.getSrcReg();
  unsigned SrcIdx = CP.isFlipped() ? CP.getDstIdx() : CP.getSrcIdx();
  CgRegister DstReg = CP.isFlipped() ? CP.getSrcReg() : CP.getDstReg();
  unsigned DstIdx = CP.isFlipped() ? CP.getSrcIdx() : CP.getDstIdx();
  if (CgRegister::isPhysicalRegister(SrcReg))
    return false;

  CgLiveInterval &SrcInt = LIS->getInterval(SrcReg);
  CgSlotIndex CopyIdx = LIS->getInstructionIndex(*CopyMI);
  CgVNInfo *ValNo = SrcInt.Query(CopyIdx).valueIn();
  if (!ValNo)
    return false;
  if (ValNo->isPHIDef() || ValNo->isUnused())
    return false;
  CgInstruction *DefMI = LIS->getInstructionFromIndex(ValNo->def);
  if (!DefMI)
    return false;
  if (DefMI->isCopyLike()) {
    IsDefCopy = true;
  ...
  if (!_llvm_workaround->isAsCheapAsAMove(*TII, *DefMI))
    return false;
  ...
  CgOperand &DstOperand = CopyMI->getOperand(0);
  CgRegister CopyDstReg = DstOperand.getReg();
  if (DstOperand.getSubReg() && !DstOperand.isUndef())
    return false;

  // If both SrcIdx and DstIdx are set, correct rematerialization would widen
  // the register substantially (beyond both source and dest size). This is
  // bad for performance since it can cascade through a function, introducing
  // many extra spills and fills (e.g. ARM can easily end up copying QQQQPR
  // registers around after a few subreg copies).
  if (SrcIdx && DstIdx)
    return false;

  const TargetRegisterClass *DefRC =
      _llvm_workaround->getRegClass(*TII, MCID, 0, TRI, *MF);
  if (!DefMI->isImplicitDef()) {
    if (DstReg.isPhysical()) {
      CgRegister NewDstReg = DstReg;
      ...
      // Theoretically, some stack frame reference could exist. Just make
      // sure it hasn't actually happened.
      assert(CgRegister::isVirtualRegister(DstReg) &&
  ...
  CgBasicBlock *MBB = CopyMI->getParent();
  CgBasicBlock::iterator MII = std::next(CgBasicBlock::iterator(CopyMI));
  _llvm_workaround->reMaterialize(*TII, *MBB, MII, DstReg, SrcIdx, *DefMI,
                                  *TRI);
  CgInstruction &NewMI = *std::prev(MII);
  ...
  if (DstIdx != 0) {
    CgOperand &DefMO = NewMI.getOperand(0);
  ...
  SmallVector<CgOperand, 4> ImplicitOps;
  ImplicitOps.reserve(CopyMI->getNumOperands() -
                      CopyMI->getDesc().getNumOperands());
  for (unsigned I = CopyMI->getDesc().getNumOperands(),
                E = CopyMI->getNumOperands();
       I != E; ++I) {
    CgOperand &MO = CopyMI->getOperand(I);
    if (MO.isReg()) {
  ...
  if (DstReg.isVirtual()) {
    ...
    CgLiveInterval &DstInt = LIS->getInterval(DstReg);
    for (CgLiveInterval::SubRange &SR : DstInt.subranges()) {
    ...
    // Add dead subregister definitions if we are defining the whole
    // register but only part of it is live. This could happen if the
    // rematerialization instruction is rematerializing more than actually
    // is used in the register. An example would be: %1 = LOAD CONSTANTS 5,
    // 8 ; Loading both 5 and 8 in different subregs ; Copying only part of
    // the register here, but the rest is undef. %2:sub_16bit<def,
    // read-undef> = COPY %1:sub_16bit
    // ==>
    ...
    if (NewIdx == 0 && DstInt.hasSubRanges()) {
      CgSlotIndex CurrIdx = LIS->getInstructionIndex(NewMI);
      CgSlotIndex DefIndex =
      ...
      if (MaxMask.any()) {
        CgLiveInterval::SubRange *SR = DstInt.createSubRange(Alloc, MaxMask);
        SR->createDeadDef(DefIndex, Alloc);
      }
    }
    ...
    if (NewIdx != 0 && DstInt.hasSubRanges()) {
      ...
      for (CgLiveInterval::SubRange &SR : DstInt.subranges()) {
        if ((SR.LaneMask & DstMask).none()) {
          ...
          // VNI is in ValNo - remove any segments in this SubRange
          // that have this ValNo
          if (CgVNInfo *RmValNo = SR.getVNInfoAt(CurrIdx.getRegSlot())) {
            SR.removeValNo(RmValNo);
            UpdatedSubRanges = true;
          }
        } else {
          // We know that this lane is defined by this instruction,
          // but at this point it may be empty because it is not used
          // by anything. This happens when updateRegDefUses adds the
          // missing lanes. Assign that lane a dead def so that the
          // interferences are properly modeled.
          if (SR.empty())
            SR.createDeadDef(DefIndex, Alloc);
        }
      }
    ...
    NewMI.addOperand(CgOperand::createRegOperand(
    ...
    // Fix that!
    CgSlotIndex NewMIIdx = LIS->getInstructionIndex(NewMI);
    for (MCRegUnitIterator Units(NewMI.getOperand(0).getReg(), TRI);
         Units.isValid(); ++Units)
      if (CgLiveRange *LR = LIS->getCachedRegUnit(*Units))
    ...
    for (CgOperand &UseMO :
      ...
      if (UseMI->isDebugInstr()) {
        if (CgRegister::isPhysicalRegister(DstReg))
          ...
          UseMO.setReg(DstReg);
        // Move the debug value directly after the def of the
        // rematerialized value in DstReg.
        MBB->splice(std::next(NewMI.getIterator()), UseMI->getParent(),
                    CgBasicBlock::iterator(UseMI));
  ...
  for (CgOperand &UseMO : MRI->use_nodbg_operands(SrcReg)) {
...
CgInstruction *CgRegisterCoalescer::eliminateUndefCopy(CgInstruction *CopyMI) {
  ...
  const CgLiveInterval &SrcLI = LIS->getInterval(SrcReg);
  ...
  if (SrcSubIdx != 0 && SrcLI.hasSubRanges()) {
    ...
    for (const CgLiveInterval::SubRange &SR : SrcLI.subranges()) {
  ...
  CgSlotIndex RegIndex = Idx.getRegSlot();
  CgLiveRange::Segment *Seg = DstLI.getSegmentContaining(RegIndex);
  assert(Seg != nullptr && "No segment for defining instruction");
  if (CgVNInfo *V = DstLI.getVNInfoAt(Seg->end)) {
    if (V->isPHIDef()) {
      CopyMI->setDesc(TII->get(TargetOpcode::IMPLICIT_DEF));
      for (unsigned i = CopyMI->getNumOperands(); i != 0; --i) {
        CgOperand &MO = CopyMI->getOperand(i - 1);
  ...
  if (CgVNInfo *PrevVNI = DstLI.getVNInfoAt(Idx)) {
    CgVNInfo *VNI = DstLI.getVNInfoAt(RegIndex);
    DstLI.MergeValueNumberInto(VNI, PrevVNI);

    // The affected subregister segments can be removed.
    LaneBitmask DstMask = TRI->getSubRegIndexLaneMask(DstSubIdx);
    for (CgLiveInterval::SubRange &SR : DstLI.subranges()) {
      if ((SR.LaneMask & DstMask).none())
        continue;

      CgVNInfo *SVNI = SR.getVNInfoAt(RegIndex);
      assert(SVNI != nullptr && CgSlotIndex::isSameInstr(SVNI->def, RegIndex));
      SR.removeValNo(SVNI);
    }
  ...
  // Mark uses as undef.
  for (CgOperand &MO : MRI->reg_nodbg_operands(DstReg)) {
    if (MO.isDef() /*|| MO.isUndef()*/)
      continue;
    const CgInstruction &MI = *MO.getParent();
    CgSlotIndex UseIdx = LIS->getInstructionIndex(MI);
    LaneBitmask UseMask = TRI->getSubRegIndexLaneMask(MO.getSubReg());
    bool isLive;
    if (!UseMask.all() && DstLI.hasSubRanges()) {
      isLive = false;
      for (const CgLiveInterval::SubRange &SR : DstLI.subranges()) {
  ...
  // ignore them.
  for (CgOperand &MO : CopyMI->operands())
...
void CgRegisterCoalescer::addUndefFlag(const CgLiveInterval &Int,
                                       CgSlotIndex UseIdx, CgOperand &MO,
                                       unsigned SubRegIdx) {
  LaneBitmask Mask = TRI->getSubRegIndexLaneMask(SubRegIdx);
  if (MO.isDef())
    Mask = ~Mask;
  bool IsUndef = true;
  for (const CgLiveInterval::SubRange &S : Int.subranges()) {
  ...
  if (IsUndef) {
    ...
    CgLiveQueryResult Q = Int.Query(UseIdx);
    if (Q.valueOut() == nullptr)
      ShrinkMainRange = true;
  }
}

void CgRegisterCoalescer::updateRegDefsUses(Register SrcReg, Register DstReg,
                                            unsigned SubIdx) {
  bool DstIsPhys = CgRegister::isPhysicalRegister(DstReg);
  CgLiveInterval *DstInt = DstIsPhys ? nullptr : &LIS->getInterval(DstReg);

  if (DstInt && DstInt->hasSubRanges() && DstReg != SrcReg) {
    for (CgOperand &MO : MRI->reg_operands(DstReg)) {
  ...
  for (CgRegisterInfo::reg_instr_iterator I = MRI->reg_instr_begin(SrcReg),
                                          E = MRI->reg_instr_end();
       I != E;) {
    CgInstruction *UseMI = &*(I++);

    // Each instruction can only be rewritten once because sub-register
    // composition is not always idempotent. When SrcReg != DstReg,
    // rewriting the UseMI operands removes them from the SrcReg use-def
    // chain, but when SrcReg is DstReg we could encounter UseMI twice if it
    // has multiple operands mentioning the virtual register.
    ...
    // If SrcReg wasn't read, it may still be the case that DstReg is
    // live-in because SrcReg is a sub-register.
    if (DstInt && !Reads && SubIdx && !UseMI->isDebugInstr())
      Reads = DstInt->liveAt(LIS->getInstructionIndex(*UseMI));

    // Replace SrcReg with DstReg in all UseMI operands.
    for (unsigned i = 0, e = Ops.size(); i != e; ++i) {
      CgOperand &MO = UseMI->getOperand(Ops[i]);

      // Adjust <undef> flags in case of sub-register joins. We don't want
      // to turn a full def into a read-modify-write sub-register def and
      // vice versa.
      if (SubIdx && MO.isDef())
        MO.setIsUndef(!Reads);

      // A subreg use of a partially undef (super) register may be a
      // complete undef use now and then has to be marked that way.
      if (MO.isUse() && !DstIsPhys) {
        ...
        if (SubUseIdx != 0 && MRI->shouldTrackSubRegLiveness(DstReg)) {
          if (!DstInt->hasSubRanges()) {
            ...
            // The unused lanes are just empty live-ranges at this
            // point. It is the caller responsibility to set the
            // proper dead segments if there is an actual dead def
            // of the unused lanes. This may happen with
            // rematerialization.
          ...
          CgSlotIndex UseIdx = MIIdx.getRegSlot(true);
...
bool CgRegisterCoalescer::canJoinPhys(const CgCoalescerPair &CP) {
  ...
  CgLiveInterval &JoinVInt = LIS->getInterval(CP.getSrcReg());
...
bool CgRegisterCoalescer::copyValueUndefInPredecessors(CgLiveRange &S,
                                                       const CgBasicBlock *MBB,
                                                       CgLiveQueryResult SLRQ) {
  for (const CgBasicBlock *Pred : MBB->predecessors()) {
    CgSlotIndex PredEnd = LIS->getMBBEndIdx(Pred);
    if (CgVNInfo *V = S.getVNInfoAt(PredEnd.getPrevSlot())) {
...
void CgRegisterCoalescer::setUndefOnPrunedSubRegUses(CgLiveInterval &LI,
                                                     CgRegister Reg,
  ...
  for (CgOperand &MO : MRI->use_nodbg_operands(Reg)) {
    unsigned SubRegIdx = MO.getSubReg();
    if (SubRegIdx == 0 || MO.isUndef())
      continue;

    LaneBitmask SubRegMask = TRI->getSubRegIndexLaneMask(SubRegIdx);
    CgSlotIndex Pos = LIS->getInstructionIndex(*MO.getParent());
...
bool CgRegisterCoalescer::joinCopy(CgInstruction *CopyMI, bool &Again) {
  Again = false;
  LLVM_DEBUG(dbgs() << LIS->getInstructionIndex(*CopyMI) << '\t' << *CopyMI);

  CgCoalescerPair CP(*TRI);
  if (!CP.setRegisters(CopyMI)) {
    LLVM_DEBUG(dbgs() << "\tNot coalescable.\n");
    return false;
  }
  // if (CP.isPartial() && LIS->getInterval(CP.getSrcReg()).size() > 1)
  //   return false; // TODO, trade off between performance and compilation time
  ...
  // Eliminate undefs.
  if (!CP.isPhys()) {
    // If this is an IMPLICIT_DEF, leave it alone, but don't try to
    // coalesce.
    if (CgInstruction *UndefMI = eliminateUndefCopy(CopyMI)) {
  ...
  if (CP.getSrcReg() == CP.getDstReg()) {
    CgLiveInterval &LI = LIS->getInterval(CP.getSrcReg());
    LLVM_DEBUG(dbgs() << "\tCopy already coalesced: " << LI << '\n');
    const CgSlotIndex CopyIdx = LIS->getInstructionIndex(*CopyMI);
    CgLiveQueryResult LRQ = LI.Query(CopyIdx);
    if (CgVNInfo *DefVNI = LRQ.valueDefined()) {
      CgVNInfo *ReadVNI = LRQ.valueIn();
      assert(ReadVNI && "No value before copy and no <undef> flag.");
      assert(ReadVNI != DefVNI && "Cannot read and define the same value.");

      // Track incoming undef lanes we need to eliminate from the
      // subrange.
      LaneBitmask PrunedLanes;
      CgBasicBlock *MBB = CopyMI->getParent();
      ...
      for (CgLiveInterval::SubRange &S : LI.subranges()) {
        CgLiveQueryResult SLRQ = S.Query(CopyIdx);
        if (CgVNInfo *SDefVNI = SLRQ.valueDefined()) {
          if (CgVNInfo *SReadVNI = SLRQ.valueIn())
            SDefVNI = S.MergeValueNumberInto(SDefVNI, SReadVNI);

          // If this copy introduced an undef subrange from an
          // incoming value, we need to eliminate the undef live in
          // values from the subrange.
  ...
  if (CP.isPhys()) {
    ...
    if (!canJoinPhys(CP)) {
      // Before giving up coalescing, if definition of source is defined
      // by trivial computation, try rematerializing it.
  ...
  // false. Otherwise, if one of the intervals being joined is a physreg, this
  // method always canonicalizes DstInt to be it.  The output "SrcInt" will
  // not have been modified, so we can use this information below to update
  // aliases.
  if (!joinIntervals(CP)) {
    // Coalescing failed.
    ...
    // Try and see if we can partially eliminate the copy by moving the copy
    // to its predecessor.
#if 0 // TODO
    if (!CP.isPartial() && !CP.isPhys())
  ...
  // other. Make sure the resulting register is set to the right register
  // class.
  if (CP.isCrossClass()) {
  ...
  // ErasedInstrs since copyCoalesceWorkList() won't add a successful join
  // back to the work list. This keeps ErasedInstrs from growing needlessly.
  ...
  if (ShrinkMask.any()) {
    CgLiveInterval &LI = LIS->getInterval(CP.getDstReg());
    for (CgLiveInterval::SubRange &S : LI.subranges()) {
  ...
  // interval. Since CP.getSrcReg() is in ToBeUpdated set and its live
  // interval is not up-to-date, need to update the merged live interval here.
...
bool CgRegisterCoalescer::joinReservedPhysReg(CgCoalescerPair &CP) {
  CgRegister DstReg = CP.getDstReg();
  CgRegister SrcReg = CP.getSrcReg();
  assert(CP.isPhys() && "Must be a physreg copy");
  assert(MRI->isReserved(DstReg) && "Not a reserved register");
  CgLiveInterval &RHS = LIS->getInterval(SrcReg);
  ...
  if (CP.isFlipped()) {
    ...
    CgInstruction &DestMI = *MRI->getVRegDef(SrcReg);
    CopyMI = &*MRI->use_instr_nodbg_begin(SrcReg);
    CgSlotIndex CopyRegIdx = LIS->getInstructionIndex(*CopyMI).getRegSlot();
    CgSlotIndex DestRegIdx = LIS->getInstructionIndex(DestMI).getRegSlot();

    if (!MRI->isConstantPhysReg(DstReg)) {
      // We checked above that there are no interfering defs of the
      // physical register. However, for this case, where we intend to
      // move up the def of the physical register, we also need to check
      // for interfering uses.
      CgSlotIndexes *Indexes = LIS->getSlotIndexes();
      for (CgSlotIndex SI = Indexes->getNextNonNullIndex(DestRegIdx);
           SI != CopyRegIdx; SI = Indexes->getNextNonNullIndex(SI)) {
        CgInstruction *MI = LIS->getInstructionFromIndex(SI);
    ...
    for (MCRegUnitIterator UI(DstReg, TRI); UI.isValid(); ++UI) {
      CgLiveRange &LR = LIS->getRegUnit(*UI);
...
class JoinVals {
  ...
  /// Whether the current CgLiveInterval tracks subregister liveness.
  const bool TrackSubRegLiveness;

  /// Values that will be present in the final live range.
  SmallVectorImpl<CgVNInfo *> &NewVNInfo;

  const CgCoalescerPair &CP;
  CgLiveIntervals *LIS;
  CgSlotIndexes *Indexes;
  const TargetRegisterInfo *TRI;

  /// Value number assignments. Maps value numbers in LI to entries in
  /// NewVNInfo. This is suitable for passing to CgLiveInterval::join().
...
public:
  ...
  enum ConflictResolution {
    ...
    /// complicates value mapping since OtherVNI maps to two different
    /// values
...
private:
  ...
  struct Val {
    ...
    /// Lanes with defined values in this register. Other lanes are undef
    /// and safe to clobber.
    LaneBitmask ValidLanes;

    /// Value in LI being redefined by this def.
    CgVNInfo *RedefVNI = nullptr;

    /// Value in the other live range that overlaps this def, if any.
    CgVNInfo *OtherVNI = nullptr;

    /// Is this value an IMPLICIT_DEF that can be erased?
    ///
    /// IMPLICIT_DEF values should only exist at the end of a basic block
    /// that is a predecessor to a phi-value. These IMPLICIT_DEF
    /// instructions can be safely erased if they are overlapping a live
    /// value in the other live interval.
    ///
  ...
  /// Set Redef if there are any partial register definitions that depend on
  /// the previous value of the register.
  LaneBitmask computeWriteLanes(const CgInstruction *DefMI, bool &Redef) const;

  /// Find the ultimate value that VNI was copied from.
  std::pair<const CgVNInfo *, Register>
  followCopyChain(const CgVNInfo *VNI) const;

  bool valuesIdentical(CgVNInfo *Value0, CgVNInfo *Value1,
                       const JoinVals &Other) const;

  /// Analyze ValNo in this live range, and set all fields of Vals[ValNo].
  /// Return a conflict resolution when possible, but leave the hard cases as
  /// CR_Unresolved.
  /// Recursively calls computeAssignment() on this and Other, guaranteeing
  /// that both OtherVNI and RedefVNI have been analyzed and mapped before
  /// returning. The recursion always goes upwards in the dominator tree,
  /// making loops impossible.
  ConflictResolution analyzeValue(unsigned ValNo, JoinVals &Other);

  /// Compute the value assignment for ValNo in RI.
  /// This may be called recursively by analyzeValue(), but never for a ValNo
  /// on the stack.
  ...
  ///   3 %dst:ssub1 = BAR       <-- Partial redef doesn't clear taint in
  ///   ssub0 4 %dst:ssub0 = COPY %src <-- Conflict resolved, ssub0 wasn't
  ///   read
  ///
  /// For each ValNo in Other that is affected, add an (EndIndex,
  /// TaintedLanes) entry to TaintedVals.
  ///
  /// Returns false if the tainted lanes extend beyond the basic block.
  bool taintExtent(
      unsigned ValNo, LaneBitmask TaintedLanes, JoinVals &Other,
      SmallVectorImpl<std::pair<CgSlotIndex, LaneBitmask>> &TaintExtent);

  /// Return true if MI uses any of the given Lanes from Reg.
  /// This does not include partial redefinitions of Reg.
  bool usesLanes(const CgInstruction &MI, Register, unsigned,
                 LaneBitmask) const;

  /// Determine if ValNo is a copy of a value number in LR or Other.LR that
  /// will be pruned:
  ///
  ///   %dst = COPY %src
...
public:
  JoinVals(CgLiveRange &LR, CgRegister Reg, unsigned SubIdx,
           LaneBitmask LaneMask, SmallVectorImpl<CgVNInfo *> &newVNInfo,
           const CgCoalescerPair &cp, CgLiveIntervals *lis,
  ...
  /// Prune the live range of values in Other.LR where they would conflict
  /// with CR_Replace values in LR. Collect end points for restoring the live
  /// range after joining.
  void pruneValues(JoinVals &Other, SmallVectorImpl<CgSlotIndex> &EndPoints,
                   bool changeInstrs);
  ...
  void pruneSubRegValues(CgLiveInterval &LI, LaneBitmask &ShrinkMask);
  ...
  /// live range LR.
  void pruneMainSegments(CgLiveInterval &LI, bool &ShrinkMainRange);
  ...
  /// Get the value assignments suitable for passing to CgLiveInterval::join.
...
LaneBitmask JoinVals::computeWriteLanes(const CgInstruction *DefMI,
                                        bool &Redef) const {
  LaneBitmask L;
  for (const CgOperand &MO : DefMI->operands()) {
...
std::pair<const CgVNInfo *, Register>
JoinVals::followCopyChain(const CgVNInfo *VNI) const {
  Register TrackReg = Reg;

  while (!VNI->isPHIDef()) {
    CgSlotIndex Def = VNI->def;
    CgInstruction *MI = Indexes->getInstructionFromIndex(Def);
    ...
    const CgLiveInterval &LI = LIS->getInterval(SrcReg);
    const CgVNInfo *ValueIn;
    // No subrange involved.
    if (!SubRangeJoin || !LI.hasSubRanges()) {
      CgLiveQueryResult LRQ = LI.Query(Def);
      ValueIn = LRQ.valueIn();
    } else {
      // Query subranges. Ensure that all matching ones take us to the
      // same def (allowing some of them to be undef).
      ValueIn = nullptr;
      for (const CgLiveInterval::SubRange &S : LI.subranges()) {
        ...
        if ((SMask & LaneMask).none())
          continue;
        CgLiveQueryResult LRQ = S.Query(Def);
        if (!ValueIn) {
          ValueIn = LRQ.valueIn();
          continue;
        }
...
bool JoinVals::valuesIdentical(CgVNInfo *Value0, CgVNInfo *Value1,
                               const JoinVals &Other) const {
  const CgVNInfo *Orig0;
  Register Reg0;
  std::tie(Orig0, Reg0) = followCopyChain(Value0);
  if (Orig0 == Value1 && Reg0 == Other.Reg)
    return true;

  const CgVNInfo *Orig1;
  Register Reg1;
  ...
  // is from the original CgLiveInterval.
...
JoinVals::ConflictResolution JoinVals::analyzeValue(unsigned ValNo,
  ...
  CgVNInfo *VNI = LR.getValNumInfo(ValNo);
  if (VNI->isUnused()) {
  ...
  const CgInstruction *DefMI = nullptr;
  if (VNI->isPHIDef()) {
    ...
    if (SubRangeJoin) {
      ...
      // If this is a read-modify-write instruction, there may be more
      // valid lanes than the ones written by this instruction. This only
      // covers partial redef operands. DefMI may have normal use operands
      // reading the register. They don't contribute valid lanes.
      //
      ...
      if (DefMI->isImplicitDef()) {
        // We normally expect IMPLICIT_DEF values to be live only until
        // the end of their block. If the value is really live longer
        // and gets pruned in another block, this flag is cleared again.
        //
        // Clearing the valid lanes is deferred until it is sure this
        // can be erased.
        V.ErasableImplicitDef = true;
      }
    }
  ...
  CgLiveQueryResult OtherLRQ = Other.LR.Query(VNI->def);
  ...
  if (CgVNInfo *OtherVNI = OtherLRQ.valueDefined()) {
    assert(CgSlotIndex::isSameInstr(VNI->def, OtherVNI->def) && "Broken LRQ");
    ...
    if (OtherVNI->def < VNI->def)
      ...
      // This is an early-clobber def overlapping a live-in value in the
      // other register. Not mergeable.
    ...
    // Allow overlapping PHI values. Any real interference would show up in
    // a predecessor, the PHI itself can't introduce any conflicts.
    if (VNI->isPHIDef())
      return CR_Merge;
  ...
  assert(!CgSlotIndex::isSameInstr(VNI->def, V.OtherVNI->def) && "Broken LRQ");
  ...
  if (OtherV.ErasableImplicitDef) {
    // Check if OtherV is an IMPLICIT_DEF that extends beyond its basic
    // block. This shouldn't normally happen, but ProcessImplicitDefs can
    // leave such IMPLICIT_DEF instructions behind, and there is nothing
    // wrong with it technically.
    //
    // When it happens, treat that IMPLICIT_DEF as a normal value, and don't
    // try to erase the IMPLICIT_DEF instruction.
    if (DefMI &&
        DefMI->getParent() != Indexes->getMBBFromIndex(V.OtherVNI->def)) {
#if 0
            LLVM_DEBUG(dbgs() << "IMPLICIT_DEF defined at " << V.OtherVNI->def
                              << " extends into "
                              << printCgBBReference(*DefMI->getParent())
                              << ", keeping it.\n");
#endif
  ...
  if (CP.isCoalescable(DefMI)) {
    // Some of the lanes copied from OtherVNI may be undef, making them
    // undef here too.
  ...
  // If the lanes written by this instruction were all undef in OtherVNI, it
  // is still safe to join the live ranges. This can't be done with a simple
  // value mapping, though - OtherVNI will map to multiple values:
  //
  ...
  //   5 QUUX killed %src
  //
  // Here OtherVNI will map to itself in [1;2), but to VNI in [2;5).
  // CR_Replace handles this complex value mapping.
  ...
  if (TrackSubRegLiveness) {
    ...
    // If OtherVNI does not have subranges, it means all the lanes of
    // OtherVNI share the same live range, so we just need to check whether
    // they have any conflict bit in their LaneMask.
    ...
    for (CgLiveInterval::SubRange &OtherSR : OtherLI.subranges()) {
      ...
      if (OtherSRQ.valueIn() && OtherSRQ.endPoint() > VNI->def) {
        // VNI is clobbering some lanes of OtherVNI, they have real
        // conflict.
        return CR_Impossible;
      }
    }
...
void JoinVals::computeAssignment(unsigned ValNo, JoinVals &Other) {
  ...
  case CR_Unresolved: {
    ...
    Val &OtherV = Other.Vals[V.OtherVNI->id];
    // We cannot erase an IMPLICIT_DEF if we don't have valid values for
    // all its lanes.
    if (OtherV.ErasableImplicitDef && TrackSubRegLiveness &&
      ...
      OtherV.ErasableImplicitDef = false;
      // The valid lanes written by the implicit_def were
      // speculatively cleared before, so make this more conservative.
      // It may be better to track this, I haven't found a testcase
      // where it matters.
...
bool JoinVals::usesLanes(const CgInstruction &MI, CgRegister Reg,
                         unsigned SubIdx, LaneBitmask Lanes) const {
...
bool JoinVals::resolveConflicts(JoinVals &Other) {
  for (unsigned i = 0, e = LR.getNumValNums(); i != e; ++i) {
    ...
    // VNI is known to clobber some lanes in OtherVNI. If we go ahead with
    // the join, those lanes will be tainted with a wrong value. Get the
    // extent of the tainted lanes.
    LaneBitmask TaintedLanes = V.WriteLanes & OtherV.ValidLanes;
    SmallVector<std::pair<CgSlotIndex, LaneBitmask>, 8> TaintExtent;
    ...
    // Now look at the instructions from VNI->def to TaintExtent
    // (inclusive).
    CgBasicBlock *MBB = Indexes->getMBBFromIndex(VNI->def);
    CgBasicBlock::iterator MI = MBB->begin();
    if (!VNI->isPHIDef()) {
      MI = CgBasicBlock::iterator(Indexes->getInstructionFromIndex(VNI->def));
      if (!VNI->def.isEarlyClobber()) {
        // No need to check the instruction defining VNI for reads.
        ++MI;
      }
    }
    assert(!CgSlotIndex::isSameInstr(VNI->def, TaintExtent.front().first) &&
           "Interference ends on VNI->def. Should have been handled "
           "earlier");
    CgInstruction *LastMI =
...
void JoinVals::pruneValues(JoinVals &Other,
                           ...
                           bool changeInstrs) {
  for (unsigned i = 0, e = LR.getNumValNums(); i != e; ++i) {
    CgSlotIndex Def = LR.getValNumInfo(i)->def;
    ...
    case CR_Replace: {
      ...
      // Check if we're replacing an IMPLICIT_DEF value. The
      // IMPLICIT_DEF instructions are only inserted to provide a
      // live-out value for PHI predecessors, so the instruction
      // should simply go away once its value has been replaced.
      Val &OtherV = Other.Vals[Vals[i].OtherVNI->id];
      bool EraseImpDef =
          OtherV.ErasableImplicitDef && OtherV.Resolution == CR_Keep;
      if (!Def.isBlock()) {
        if (changeInstrs) {
          // Remove <def,read-undef> flags. This def is now a
          // partial redef. Also remove dead flags since the
          // joined live range will continue past this
          // instruction.
          for (CgOperand &MO :
        ...
        // This value will reach instructions below, but we need to
        // make sure the live range also reaches the instruction at
        // Def.
        if (!EraseImpDef)
    ...
    case CR_Merge:
      if (isPrunedValue(i, Other)) {
        // This value is ultimately a copy of a pruned value in LR
        // or Other.LR. We can no longer trust the value mapping
        // computed by computeAssignment(), the value that was
        // originally copied could have been replaced.
...
// to handle).
static bool isLiveThrough(const CgLiveQueryResult Q) {
...
void JoinVals::pruneSubRegValues(CgLiveInterval &LI, LaneBitmask &ShrinkMask) {
    ...
    for (CgLiveInterval::SubRange &S : LI.subranges()) {
      ...
      if (ValueOut != nullptr &&
        ...
        if (V.Identical && S.Query(OtherDef).valueOutOrDead()) {
          // If V is identical to V.OtherVNI (and S was live at
          // OtherDef), then we can't simply prune V from S. V needs
          // to be replaced with V.OtherVNI.
          LIS->extendToIndices(S, EndPoints);
        }

        // We may need to eliminate the subrange if the copy introduced
        // a live out undef value.
...
static bool isDefInSubRange(CgLiveInterval &LI, CgSlotIndex Def) {
  for (CgLiveInterval::SubRange &SR : LI.subranges()) {
    if (CgVNInfo *VNI = SR.Query(Def).valueOutOrDead())
      if (VNI->def == Def)
        return true;
  }
  return false;
...
void JoinVals::pruneMainSegments(CgLiveInterval &LI, bool &ShrinkMainRange) {
  assert(&static_cast<CgLiveRange &>(LI) == &LR);
...
void JoinVals::eraseInstrs(SmallPtrSetImpl<CgInstruction *> &ErasedInstrs,
  ...
  for (unsigned i = 0, e = LR.getNumValNums(); i != e; ++i) {
    ...
    switch (Vals[i].Resolution) {
    case CR_Keep: {
      // If an IMPLICIT_DEF value is pruned, it doesn't serve a
      // purpose any longer. The IMPLICIT_DEF instructions are only
      // inserted by PHIElimination to guarantee that all PHI
      // predecessors have a value.
      if (!Vals[i].ErasableImplicitDef || !Vals[i].Pruned)
        break;
      // Remove value number i from LR.
      // For intervals with subranges, removing a segment from the
      // main range may require extending the previous segment: for
      // each definition of a subregister, there will be a
      // corresponding def in the main range. That def may fall in the
      // middle of a segment from another subrange. In such cases,
      // removing this def from the main range must be complemented by
      // extending the main range to account for the liveness of the
      // other subrange. The new end point of the main range segment
      // to be extended.
      CgSlotIndex NewEnd;
      if (LI != nullptr) {
        CgLiveRange::iterator I = LR.FindSegmentContaining(Def);
        assert(I != LR.end());
        // Do not extend beyond the end of the segment being
        // removed. The segment may have been pruned in preparation
        // for joining live ranges.
        NewEnd = I->end;
      }

      LR.removeValNo(VNI);
      // Note that this VNInfo is reused and still referenced in
      // NewVNInfo, make it appear like an unused value number.
      VNI->markUnused();

      if (LI != nullptr && LI->hasSubRanges()) {
        assert(static_cast<CgLiveRange *>(LI) == &LR);
        // Determine the end point based on the subrange
        // information: minimum of (earliest def of next segment,
        //             latest end point of containing segment)
        CgSlotIndex ED, LE;
        for (CgLiveInterval::SubRange &SR : LI->subranges()) {
          CgLiveRange::iterator I = SR.find(Def);
          if (I == SR.end())
            continue;
          if (I->start > Def)
        ...
        // We only want to do the extension if there was a subrange
        // that was live across Def.
        if (LE.isValid()) {
          CgLiveRange::iterator S = LR.find(Def);
          if (S != LR.begin())
...
void CgRegisterCoalescer::joinSubRegRanges(CgLiveRange &LRange,
                                           CgLiveRange &RRange,
                                           LaneBitmask LaneMask,
                                           const CgCoalescerPair &CP) {
  SmallVector<CgVNInfo *, 16> NewVNInfo;
  ...
  // We should be able to resolve all conflicts here as we could successfully
  // do it on the mainrange already. There is however a problem when multiple
  // ranges get mapped to the "overflow" lane mask bit which creates
  // unexpected interferences.
  if (!LHSVals.mapValues(RHSVals) || !RHSVals.mapValues(LHSVals)) {
    // We already determined that it is legal to merge the intervals, so
  ...
  // The merging algorithm in CgLiveInterval::join() can't handle conflicting
...
void CgRegisterCoalescer::mergeSubRangeInto(CgLiveInterval &LI,
                                            const CgLiveRange &ToMerge,
                                            LaneBitmask LaneMask,
                                            CgCoalescerPair &CP,
  ...
  LI.refineSubRanges(
      Allocator, LaneMask,
      [this, &Allocator, &ToMerge, &CP](CgLiveInterval::SubRange &SR) {
        if (SR.empty()) {
          SR.assign(ToMerge, Allocator);
        } else {
          // joinSubRegRange() destroys the merged range, so we need a
          // copy.
          CgLiveRange RangeCopy(ToMerge, Allocator);
...
bool CgRegisterCoalescer::isHighCostLiveInterval(CgLiveInterval &LI) {
  // if (LI.valnos.size() < LargeIntervalSizeThreshold)
  if (LI.valnos.size() < 100)
    return false;
...
bool CgRegisterCoalescer::joinVirtRegs(CgCoalescerPair &CP) {
  SmallVector<CgVNInfo *, 16> NewVNInfo;
  ...
  if (RHS.hasSubRanges() || LHS.hasSubRanges()) {
    ...
    // Transform lanemasks from the LHS to masks in the coalesced register
    // and create initial subranges if necessary.
    unsigned DstIdx = CP.getDstIdx();
    if (!LHS.hasSubRanges()) {
      ...
      for (CgLiveInterval::SubRange &R : LHS.subranges()) {
    ...
    // Determine lanemasks of RHS in the coalesced register and merge
    // subranges.
    unsigned SrcIdx = CP.getSrcIdx();
    if (!RHS.hasSubRanges()) {
      ...
      for (CgLiveInterval::SubRange &R : RHS.subranges()) {
  ...
  if (RegIt != RegToPHIIdx.end()) {
    // Iterate over all the debug instruction numbers assigned this
    // register.
    for (unsigned InstID : RegIt->second) {
      auto PHIIt = PHIValToPos.find(InstID);
      assert(PHIIt != PHIValToPos.end());
      const CgSlotIndex &SI = PHIIt->second.SI;
      ...
      if (CP.getSrcIdx() != 0 || CP.getDstIdx() != 0)
        // If we're moving between different subregisters, ignore this
        // join. The PHI will not get a location, dropping variable
        // locations.
    ...
    // Rebuild the register index in RegToPHIIdx to account for PHIs
    // tracking different VRegs now. Copy old collection of debug
    // instruction numbers and erase the old one:
    auto InstrNums = RegIt->second;
    RegToPHIIdx.erase(RegIt);

    // There might already be PHIs being tracked in the destination VReg.
    // Insert into an existing tracking collection, or insert a new one.
  ...
  if (!EndPoints.empty()) {
    ...
    LIS->extendToIndices((CgLiveRange &)LHS, EndPoints);
  }

  return true;
}

bool CgRegisterCoalescer::joinIntervals(CgCoalescerPair &CP) {
  return CP.isPhys() ? joinReservedPhysReg(CP) : joinVirtRegs(CP);
}

void CgRegisterCoalescer::buildVRegToDbgValueMap(CgFunction &MF) {
  const CgSlotIndexes &Slots = *LIS->getSlotIndexes();
  SmallVector<CgInstruction *, 8> ToInsert;

  // After collecting a block of DBG_VALUEs into ToInsert, enter them into the
  // vreg => DbgValueLoc map.
  auto CloseNewDVRange = [this, &ToInsert](CgSlotIndex Slot) {
    for (auto *X : ToInsert) {
  ...
  for (auto &MBB : MF) {
    CgSlotIndex CurrentSlot = Slots.getMBBStartIdx(&*MBB);

    for (auto &MI : *MBB) {
      if (MI.isDebugValue()) {
        if (any_of(MI.debug_operands(), [](const CgOperand &MO) {
...
void CgRegisterCoalescer::checkMergingChangesDbgValues(CgCoalescerPair &CP,
                                                       CgLiveRange &LHS,
                                                       JoinVals &LHSVals,
                                                       CgLiveRange &RHS,
                                                       JoinVals &RHSVals) {
  auto ScanForDstReg = [&](CgRegister Reg) {
    checkMergingChangesDbgValuesImpl(Reg, RHS, LHS, LHSVals);
  };

  auto ScanForSrcReg = [&](CgRegister Reg) {
    checkMergingChangesDbgValuesImpl(Reg, LHS, RHS, RHSVals);
  };

  // Scan for potentially unsound DBG_VALUEs: examine first the register
  // number Reg, and then any other vregs that may have been merged into  it.
  auto PerformScan = [this](CgRegister Reg,
                            std::function<void(CgRegister)> Func) {
    Func(Reg);
    if (DbgMergedVRegNums.count(Reg))
      for (CgRegister X : DbgMergedVRegNums[Reg])
        Func(X);
  };
...
void CgRegisterCoalescer::checkMergingChangesDbgValuesImpl(CgRegister Reg,
                                                           CgLiveRange &OtherLR,
                                                           CgLiveRange &RegLR,
                                                           JoinVals &RegVals) {
  ...
  auto ShouldUndef = [&RegVals, &RegLR, &LastUndefResult,
                      &LastUndefIdx](CgSlotIndex Idx) -> bool {
    ...
    // If the other range was live, and Reg's was not, the register
    // coalescer will not have tried to resolve any conflicts. We don't know
    // whether the DBG_VALUE will refer to the same value number, so it must
    // be made undef.
    auto OtherIt = RegLR.find(Idx);
    if (OtherIt == RegLR.end())
      return true;

    // Both the registers were live: examine the conflict resolution record
    // for the value number Reg refers to. CR_Keep meant that this value
    // number "won" and the merged register definitely refers to that value.
    // CR_Erase means the value number was a redundant copy of the other
    // value, which was coalesced and Reg deleted. It's safe to refer to the
    // other register (which will be the source of the copy).
  ...
  while (DbgValueSetIt != DbgValueSet.end() && SegmentIt != OtherLR.end()) {
    if (DbgValueSetIt->first < SegmentIt->end) {
      // "Other" is live and there is a DBG_VALUE of Reg: test if we
      // should set it undef.
      if (DbgValueSetIt->first >= SegmentIt->start) {
        ...
        if (HasReg && ShouldUndefReg) {
          // Mark undef, erase record of this DBG_VALUE to avoid
          // revisiting.
...
struct MBBPriorityInfo {
  ...
  MBBPriorityInfo(CgBasicBlock *mbb, unsigned depth, bool issplit)
...
static bool isLocalCopy(CgInstruction *Copy, const CgLiveIntervals *LIS) {
  if (!Copy->isCopy())
    return false;
...
void CgRegisterCoalescer::lateLiveIntervalUpdate() {
  for (CgRegister reg : ToBeUpdated) {
...
bool CgRegisterCoalescer::copyCoalesceWorkList(
    MutableArrayRef<CgInstruction *> CurrList) {
  bool Progress = false;
  for (CgInstruction *&MI : CurrList) {
    if (!MI)
      continue;
    // Skip instruction pointers that have already been erased, for example
...
static bool isTerminalReg(CgRegister DstReg, const CgInstruction &Copy,
                          const CgRegisterInfo *MRI) {
  assert(Copy.isCopyLike());
  // Check if the destination of this copy as any other affinity.
  for (const CgInstruction &MI : MRI->reg_nodbg_instructions(DstReg))
    if (&MI != &Copy && MI.isCopyLike())
      return false;
  return true;
}

bool CgRegisterCoalescer::applyTerminalRule(const CgInstruction &Copy) const {
  assert(Copy.isCopyLike());
  // if (!UseTerminalRule)
  if (!false)
    return false;
  ...
  const CgBasicBlock *OrigBB = Copy.getParent();
  const CgLiveInterval &DstLI = LIS->getInterval(DstReg);
  for (const CgInstruction &MI : MRI->reg_nodbg_instructions(SrcReg)) {
...
void CgRegisterCoalescer::copyCoalesceInMBB(CgBasicBlock *MBB) {
  LLVM_DEBUG(dbgs() << MBB->getName() << ":\n");

  // Collect all copy-like instructions in MBB. Don't start coalescing
  // anything yet, it might invalidate the iterator.
  const unsigned PrevSize = WorkList.size();
  if (JoinGlobalCopies) {
    SmallVector<CgInstruction *, 2> LocalTerminals;
    SmallVector<CgInstruction *, 2> GlobalTerminals;
    // Coalesce copies bottom-up to coalesce local defs before local uses.
    // They are not inherently easier to resolve, but slightly preferable
    // until we have local live range splitting. In particular this is
    // required by cmp+jmp macro fusion.
    ...
    // Append the copies evicted by the terminal rule at the end of the
    // list.
    LocalWorkList.append(LocalTerminals.begin(), LocalTerminals.end());
    WorkList.append(GlobalTerminals.begin(), GlobalTerminals.end());
  } else {
    SmallVector<CgInstruction *, 2> Terminals;
    for (CgInstruction &MII : *MBB)
      if (MII.isCopyLike()) {
  ...
  MutableArrayRef<CgInstruction *> CurrList(WorkList.begin() + PrevSize,
                                            WorkList.end());
...
void CgRegisterCoalescer::coalesceLocals() {
...
void CgRegisterCoalescer::joinAllIntervals() {
  ...
  // join until we make no progress.
  while (copyCoalesceWorkList(WorkList))
    /* empty */;
  lateLiveIntervalUpdate();
}

void CgRegisterCoalescer::releaseMemory() {
  ErasedInstrs.clear();
  WorkList.clear();
  DeadDefs.clear();
  InflateRegs.clear();
...
bool CgRegisterCoalescer::runOnCgFunction(CgFunction &fn) {
  LLVM_DEBUG(dbgs() << "********** SIMPLE REGISTER COALESCING **********\n"
                    << "********** Function: " << fn.getName() << '\n');

#if 0
    // Variables changed between a setjmp and a longjump can have undefined
    // value after the longjmp. This behaviour can be observed if such a
    // variable is spilled, so longjmp won't restore the value in the spill
    // slot. RegisterCoalescer should not run in functions with a setjmp to
    // avoid merging such undefined variables with predictable ones.
    //
  ...
  if (true)
    ...
    CgSlotIndexes *Slots = LIS->getSlotIndexes();
    for (const auto &DebugPHI : MF->DebugPHIPositions) {
        CgBasicBlock *MBB = DebugPHI.second.MBB;
        CgRegister Reg = DebugPHI.second.Reg;
        unsigned SubReg = DebugPHI.second.SubReg;
        CgSlotIndex SI = Slots->getMBBStartIdx(MBB);
  ...
  // either be enabled unconditionally or replaced by a more general live
  // range splitting optimization. JoinSplitEdges = EnableJoinSplits;
  JoinSplitEdges = false;
...
// if (VerifyCoalescing)
  ...
  RegClassInfo.runOnCgFunction(fn);
  ...
  for (unsigned i = 0, e = InflateRegs.size(); i != e; ++i) {
    ...
    if (MRI->recomputeRegClass(Reg)) {
      ...
      CgLiveInterval &LI = LIS->getInterval(Reg);
      if (LI.hasSubRanges()) {
        // If the inflated register class does not support subregisters
        // anymore remove the subranges.
  ...
  llvm::dbgs() << "\n########## CgIR Dump After Register Coalescing "
                  "##########\n\n";
  MF->dump();
#endif
...
void CgRegisterCoalescer::print(raw_ostream &O) const { LIS->print(O); }

//===- CgSpillPlacement.h - Optimal Spill Code Placement ---------*- C++
//-*--===//
//
...
//
// The runOnCgFunction() method only precomputes some profiling information
...
class CgSpillPlacement : public NonCopyable {
  struct Node;
  const CgFunction *MF;
  const CgEdgeBundles *bundles;
...
public:
  CgSpillPlacement(CgFunction &MF) { runOnCgFunction(MF); }
  ~CgSpillPlacement() { releaseMemory(); }

...
#include "compiler/cgir/pass/live_range_utils.h"
...
protected:
  CalcLiveRangeUtilBase(CgLiveRange *LR) : LR(LR) {}

public:
  using Segment = CgLiveRange::Segment;
  using iterator = IteratorT;

  /// A counterpart of CgLiveRange::createDeadDef: Make sure the range has a
  ...
  CgVNInfo *createDeadDef(CgSlotIndex Def, CgVNInfo::Allocator *VNInfoAllocator,
                          CgVNInfo *ForVNI) {
    ...
    if (I == segments().end()) {
      CgVNInfo *VNI = ForVNI ? ForVNI : LR->getNextValue(Def, *VNInfoAllocator);
      impl().insertAtEnd(Segment(Def, Def.getDeadSlot(), VNI));
      return VNI;
    }

    Segment *S = segmentAt(I);
    if (CgSlotIndex::isSameInstr(Def, S->start)) {
      ...
      // same register on an instruction. It doesn't make a lot of sense,
      // but it is possible to specify in inline assembly.
      //
    ...
    assert(CgSlotIndex::isEarlierInstr(Def, S->start) && "Already live at def");
    CgVNInfo *VNI = ForVNI ? ForVNI : LR->getNextValue(Def, *VNInfoAllocator);
    segments().insert(I, Segment(Def, Def.getDeadSlot(), VNI));
    return VNI;
  }

  CgVNInfo *extendInBlock(CgSlotIndex StartIdx, CgSlotIndex Use) {
    if (segments().empty())
      return nullptr;
  ...
  std::pair<CgVNInfo *, bool> extendInBlock(ArrayRef<CgSlotIndex> Undefs,
    ...
    CgSlotIndex BeforeUse = Use.getPrevSlot();
  ...
  void extendSegmentEndTo(iterator I, CgSlotIndex NewEnd) {
    assert(I != segments().end() && "Not a valid segment!");
    Segment *S = segmentAt(I);
    CgVNInfo *ValNo = I->valno;
    ...
    // If NewEnd was in the middle of a segment, make sure to get its
    // endpoint.
    S->end = std::max(NewEnd, std::prev(MergeTo)->end);

    // If the newly formed segment now touches the segment after it and if
    // they have the same value number, merge the two segments into one
    // segment.
  ...
  iterator extendSegmentStartTo(iterator I, CgSlotIndex NewStart) {
    assert(I != segments().end() && "Not a valid segment!");
    Segment *S = segmentAt(I);
    CgVNInfo *ValNo = I->valno;
  ...
  iterator addSegment(Segment S) {
    CgSlotIndex Start = S.start, End = S.end;
    iterator I = impl().findInsertPos(S);

    // If the inserted segment starts in the middle or right at the end of
    // another segment, just extend that segment to contain the segment of
    // S.
    if (I != segments().begin()) {
      ...
      if (S.valno == B->valno) {
        ...
        // Check to make sure that we are not overlapping two live
        // segments with different valno's.
        assert(B->end <= Start &&
               "Cannot overlap two segments with differing ValID's"
               " (did you def the same reg twice in a CgInstruction?)");
      }
    }
    ...
    if (I != segments().end()) {
      if (S.valno == I->valno) {
        if (I->start <= End) {
          ...
          // If S is a complete superset of a segment, we may need to
          // grow its endpoint as well.
          if (End > I->end)
...
using CalcLiveRangeUtilVectorBase =
    CalcLiveRangeUtilBase<CalcLiveRangeUtilVector, CgLiveRange::iterator,
                          CgLiveRange::Segments>;

class CalcLiveRangeUtilVector : public CalcLiveRangeUtilVectorBase {
public:
  CalcLiveRangeUtilVector(CgLiveRange *LR) : CalcLiveRangeUtilVectorBase(LR) {}

private:
  friend CalcLiveRangeUtilVectorBase;

  CgLiveRange::Segments &segmentsColl() { return LR->segments; }

  void insertAtEnd(const Segment &S) { LR->segments.push_back(S); }

  iterator find(CgSlotIndex Pos) { return LR->find(Pos); }
...
using CalcLiveRangeUtilSetBase =
    CalcLiveRangeUtilBase<CalcLiveRangeUtilSet,
                          CgLiveRange::SegmentSet::iterator,
                          CgLiveRange::SegmentSet>;

class CalcLiveRangeUtilSet : public CalcLiveRangeUtilSetBase {
public:
  CalcLiveRangeUtilSet(CgLiveRange *LR) : CalcLiveRangeUtilSetBase(LR) {}

private:
  friend CalcLiveRangeUtilSetBase;

  CgLiveRange::SegmentSet &segmentsColl() { return *LR->segmentSet; }
...
CgLiveRange::iterator CgLiveRange::find(CgSlotIndex Pos) {
  return llvm::partition_point(*this,
                               [&](const Segment &X) { return X.end <= Pos; });
}

CgVNInfo *CgLiveRange::createDeadDef(CgSlotIndex Def,
                                     CgVNInfo::Allocator &VNIAlloc) {
...
CgVNInfo *CgLiveRange::createDeadDef(CgVNInfo *VNI) {
  // Use the segment set, if it is available.
  if (segmentSet != nullptr)
...
// A and C.
//
bool CgLiveRange::overlapsFrom(const CgLiveRange &other,
...
bool CgLiveRange::overlaps(const CgLiveRange &Other, const CgCoalescerPair &CP,
  ...
  while (true) {
    ...
    if (J->start < I->end) {
      ...
      CgSlotIndex Def = std::max(I->start, J->start);
...
/// by [Start, End).
bool CgLiveRange::overlaps(CgSlotIndex Start, CgSlotIndex End) const {
  assert(Start < End && "Invalid range");
  const_iterator I = lower_bound(*this, End);
  return I != begin() && (--I)->end > Start;
}

bool CgLiveRange::covers(const CgLiveRange &Other) const {
  if (empty())
    return Other.empty();
...
void CgLiveRange::markValNoForDeletion(CgVNInfo *ValNo) {
...
void CgLiveRange::RenumberValues() {
  SmallPtrSet<CgVNInfo *, 8> Seen;
  valnos.clear();
  for (const Segment &S : segments) {
    CgVNInfo *VNI = S.valno;
...
void CgLiveRange::addSegmentToSet(Segment S) {
  CalcLiveRangeUtilSet(this).addSegment(S);
}

CgLiveRange::iterator CgLiveRange::addSegment(Segment S) {
  // Use the segment set, if it is available.
  if (segmentSet != nullptr) {
    addSegmentToSet(S);
    return end();
...
void CgLiveRange::append(const Segment S) {
  // Check that the segment belongs to the back of the list.
  assert(segments.empty() || segments.back().end <= S.start);
  segments.push_back(S);
}

std::pair<CgVNInfo *, bool>
CgLiveRange::extendInBlock(ArrayRef<CgSlotIndex> Undefs, CgSlotIndex StartIdx,
                           CgSlotIndex Kill) {
  // Use the segment set, if it is available.
  if (segmentSet != nullptr)
...
CgVNInfo *CgLiveRange::extendInBlock(CgSlotIndex StartIdx, CgSlotIndex Kill) {
  // Use the segment set, if it is available.
  if (segmentSet != nullptr)
...
void CgLiveRange::removeSegment(CgSlotIndex Start, CgSlotIndex End,
...
CgLiveRange::iterator CgLiveRange::removeSegment(iterator I,
                                                 bool RemoveDeadValNo) {
  CgVNInfo *ValNo = I->valno;
  I = segments.erase(I);
  if (RemoveDeadValNo)
...
void CgLiveRange::removeValNoIfDead(CgVNInfo *ValNo) {
...
void CgLiveRange::removeValNo(CgVNInfo *ValNo) {
  if (empty())
    return;
...
void CgLiveRange::join(CgLiveRange &Other, const int *LHSValNoAssignments,
  ...
  // If we have to apply a mapping to our base range assignment, rewrite it
  // now.
  if (MustMapCurValNos && !empty()) {
    ...
    for (iterator I = std::next(OutIt), E = end(); I != E; ++I) {
      CgVNInfo *nextValNo = NewVNInfo[LHSValNoAssignments[I->valno->id]];
      assert(nextValNo && "Huh?");

      // If this live range has the same value # as its immediate
      // predecessor, and if they are neighbors, remove one Segment.  This
      // happens when we have [0,4:0)[4,7:1) and map 0/1 onto the same
      // value #.
  ...
  // Rewrite Other values before changing the CgVNInfo ids.
  // This can leave Other in an invalid state because we're not coalescing
  // touching segments that now have identical values. That's OK since Other
  // is not supposed to be valid after calling join();
  for (Segment &S : Other.segments)
    S.valno = NewVNInfo[RHSValNoAssignments[S.valno->id]];

  // Update val# info. Renumber them and make sure they all belong to this
  // CgLiveRange now. Also remove dead val#'s.
  unsigned NumValNos = 0;
  for (unsigned i = 0; i < NumNewVals; ++i) {
    CgVNInfo *VNI = NewVNInfo[i];
    if (VNI) {
  ...
  CgLiveRangeUpdater Updater(this);
  for (Segment &S : Other.segments)
    Updater.add(S);
}
...
void CgLiveRange::MergeSegmentsInAsValue(const CgLiveRange &RHS,
                                         CgVNInfo *LHSValNo) {
  CgLiveRangeUpdater Updater(this);
...
void CgLiveRange::MergeValueInAsValue(const CgLiveRange &RHS,
                                      const CgVNInfo *RHSValNo,
                                      CgVNInfo *LHSValNo) {
  CgLiveRangeUpdater Updater(this);
  for (const Segment &S : RHS.segments)
    if (S.valno == RHSValNo)
...
CgVNInfo *CgLiveRange::MergeValueNumberInto(CgVNInfo *V1, CgVNInfo *V2) {
  ...
  for (iterator I = begin(); I != end();) {
    ...
    // Okay, we found a V1 live range.  If it had a previous, touching, V2
    // live range, extend it.
    if (S != begin()) {
      iterator Prev = S - 1;
    ...
    // Okay, now we have a V1 or V2 live range that is maximally merged
    // forward. Ensure that it is a V2 live-range.
    S->valno = V2;
...
void CgLiveRange::flushSegmentSet() {
...
bool CgLiveRange::isLiveAtIndexes(ArrayRef<CgSlotIndex> Slots) const {
  ArrayRef<CgSlotIndex>::iterator SlotI = Slots.begin();
...
void CgLiveInterval::freeSubRange(SubRange *S) {
  S->~SubRange();
  // Memory was allocated with BumpPtr allocator and is not freed here.
}

void CgLiveInterval::removeEmptySubRanges() {
...
void CgLiveInterval::clearSubRanges() {
...
/// from \p SR.
static void stripValuesNotDefiningMask(unsigned Reg,
                                       CgLiveInterval::SubRange &SR,
  ...
  SmallVector<CgVNInfo *, 8> ToBeRemoved;
  for (CgVNInfo *VNI : SR.valnos) {
    ...
    if (VNI->isPHIDef())
      continue;
    const CgInstruction *MI = Indexes.getInstructionFromIndex(VNI->def);
    assert(MI && "Cannot find the definition of a value");
    bool hasDef = false;
    for (const CgOperand &MOI : *MI) {
...
void CgLiveInterval::refineSubRanges(
  ...
  for (SubRange &SR : subranges()) {
    ...
    if (SRMask == Matching) {
      ...
      // We have to split the subrange into a matching and non-matching
      // part. Reduce lanemask of existing lane to non-matching part.
...
unsigned CgLiveInterval::getSize() const {
  unsigned Sum = 0;
  for (const Segment &S : segments)
...
void CgLiveInterval::computeSubRangeUndefs(SmallVectorImpl<CgSlotIndex> &Undefs,
                                           LaneBitmask LaneMask,
  ...
  for (const CgOperand &MO : MRI.def_operands(reg())) {
    ...
    if ((UndefMask & LaneMask).any()) {
      ...
      CgSlotIndex Pos =
          Indexes.getInstructionIndex(MI).getRegSlot(EarlyClobber);
      Undefs.push_back(Pos);
    }
  }
}

raw_ostream &llvm::operator<<(raw_ostream &OS, const CgLiveRange::Segment &S) {
  return OS << '[' << S.start << ',' << S.end << ':' << S.valno->id << ')';
}

#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
LLVM_DUMP_METHOD void CgLiveRange::Segment::dump() const {
  dbgs() << *this << '\n';
}
#endif

void CgLiveRange::print(raw_ostream &OS) const {
  if (empty())
    OS << "EMPTY";
  else {
    for (const Segment &S : segments) {
      OS << S;
      assert(S.valno == getValNumInfo(S.valno->id) && "Bad CgVNInfo");
    }
  }
...
void CgLiveInterval::SubRange::print(raw_ostream &OS) const {
  OS << "  L" << PrintLaneMask(LaneMask) << ' '
     << static_cast<const CgLiveRange &>(*this);
}

void CgLiveInterval::print(raw_ostream &OS) const {
  OS << printReg(reg()) << ' ';
  super::print(OS);
  // Print subranges
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
LLVM_DUMP_METHOD void CgLiveRange::dump() const { dbgs() << *this << '\n'; }

LLVM_DUMP_METHOD void CgLiveInterval::SubRange::dump() const {
  dbgs() << *this << '\n';
}

LLVM_DUMP_METHOD void CgLiveInterval::dump() const { dbgs() << *this << '\n'; }
#endif

#ifndef NDEBUG
void CgLiveRange::verify() const {
...
void CgLiveInterval::verify(const CgRegisterInfo *MRI) const {
  super::verify();
...
#if 0
//===----------------------------------------------------------------------===//
//                           CgLiveRangeUpdater class
//===----------------------------------------------------------------------===//
//
// The CgLiveRangeUpdater class always maintains these invariants:
//
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
void
CgLiveRangeUpdater::print(raw_ostream &OS) const
{
    if (!isDirty()) {
        if (LR)
            OS << "Clean updater: " << *LR << '\n';
...
static inline bool coalescable(const CgLiveRange::Segment &A,
                               const CgLiveRange::Segment &B) {
...
void CgLiveRangeUpdater::add(CgLiveRange::Segment Seg) {
...
void CgLiveRangeUpdater::mergeSpills() {
  ...
  CgLiveRange::iterator Src = WriteI;
  CgLiveRange::iterator Dst = Src + NumMoved;
  CgLiveRange::iterator SpillSrc = Spills.end();
  CgLiveRange::iterator B = LR->begin();
...
void CgLiveRangeUpdater::flush() {
  if (!isDirty())
    return;
  // Clear the dirty state.
  LastStart = CgSlotIndex();
  ...
  if (GapSize < Spills.size()) {
    ...
    LR->segments.insert(ReadI, Spills.size() - GapSize, CgLiveRange::Segment());
...
unsigned ConnectedVNInfoEqClasses::Classify(const CgLiveRange &LR) {
  // Create initial equivalence classes.
  EqClass.clear();
  EqClass.grow(LR.getNumValNums());

  const CgVNInfo *used = nullptr, *unused = nullptr;

  // Determine connections.
  for (const CgVNInfo *VNI : LR.valnos) {
    ...
    used = VNI;
    if (VNI->isPHIDef()) {
      const CgBasicBlock *MBB = LIS.getMBBFromIndex(VNI->def);
      assert(MBB && "Phi-def has no defining MBB");
      // Connect to values live out of predecessors.
      for (CgBasicBlock *Pred : MBB->predecessors())
        if (const CgVNInfo *PVNI = LR.getVNInfoBefore(LIS.getMBBEndIdx(Pred)))
          EqClass.join(VNI->id, PVNI->id);
    } else {
      // Normal value defined by an instruction. Check for two-addr redef.
      // FIXME: This could be coincidental. Should we really check for a
      // tied operand constraint? Note that VNI->def may be a use slot for
      // an early clobber def.
      if (const CgVNInfo *UVNI = LR.getVNInfoBefore(VNI->def))
...
void ConnectedVNInfoEqClasses::Distribute(CgLiveInterval &LI,
                                          CgLiveInterval *LIV[],
                                          CgRegisterInfo &MRI) {
  // Rewrite instructions.
  for (CgOperand &MO : llvm::make_early_inc_range(MRI.reg_operands(LI.reg()))) {
    CgInstruction *MI = MO.getParent();
    const CgVNInfo *VNI;
    // if (MI->isDebugValue()) {
    //     // DBG_VALUE instructions don't have slot indexes, so get the
    //     index
    //     // of the instruction before them. The value is defined there
    //     too. CgSlotIndex Idx =
    //     LIS.getSlotIndexes()->getIndexBefore(*MI); VNI =
    //     LI.Query(Idx).valueOut();
    // }
    // else {
    ...
    // will be NULL. If the use is tied to a def, VNI will be the
    // defined value.
    if (!VNI)
      continue;
  ...
  if (LI.hasSubRanges()) {
    ...
    SmallVector<CgLiveInterval::SubRange *, 8> SubRanges;
    BumpPtrAllocator &Allocator = LIS.getVNInfoAllocator();
    for (CgLiveInterval::SubRange &SR : LI.subranges()) {
      // Create new subranges in the split intervals and construct a
      // mapping for the VNInfos in the subrange.
      ...
      for (unsigned I = 0; I < NumValNos; ++I) {
        const CgVNInfo &VNI = *SR.valnos[I];
        unsigned ComponentNum;
        if (VNI.isUnused()) {
          ComponentNum = 0;
        } else {
          const CgVNInfo *MainRangeVNI = LI.getVNInfoAt(VNI.def);
          assert(MainRangeVNI != nullptr &&
                 "SubRange def must have corresponding main "
                 "range "
                 "def");

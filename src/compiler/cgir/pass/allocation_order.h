...
//===-- llvm/CodeGen/CgAllocationOrder.h - Allocation Order -*- C++
//-*-------===//
//
...
class LLVM_LIBRARY_VISIBILITY CgAllocationOrder {
  const SmallVector<MCPhysReg, 16> Hints;
  ArrayRef<MCPhysReg> Order;
  // How far into the Order we can iterate. This is 0 if the CgAllocationOrder
  // is constructed with HardHints = true, Order.size() otherwise. While
  // technically a size_t, it will participate in comparisons with the
  // Iterator's Pos, which must be signed, so it's typed here as signed, too,
  // to avoid warnings and under the assumption that the size of Order is
  // relatively small.
  // IterationLimit defines an invalid iterator position.
  const int IterationLimit;

public:
  /// Forward iterator for an CgAllocationOrder.
  class Iterator final {
    const CgAllocationOrder &AO;
    int Pos = 0;

  public:
    Iterator(const CgAllocationOrder &AO, int Pos) : AO(AO), Pos(Pos) {}
  ...
  /// Create a new CgAllocationOrder for VirtReg.
  /// @param VirtReg      Virtual register to allocate for.
  /// @param VRM          Virtual register map for function.
  /// @param RegClassInfo Information about reserved and allocatable
  /// registers.
  static CgAllocationOrder create(unsigned VirtReg, const CgVirtRegMap &VRM,
                                  const CgRegisterClassInfo &RegClassInfo,
                                  const CgLiveRegMatrix *Matrix);

  /// Create an CgAllocationOrder given the Hits, Order, and HardHits values.
  /// Use the create method above - the ctor is for unittests.
  CgAllocationOrder(SmallVector<MCPhysReg, 16> &&Hints,

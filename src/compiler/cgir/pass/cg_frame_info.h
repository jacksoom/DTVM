...
/// The CgFrameInfo class represents an abstract stack frame until
...
public:
  ...
  enum SSPLayoutKind {
    ...
    SSPLK_SmallArray, ///< Array or nested array < SSP-buffer-size. 2nd
...
private:
  ...
  struct StackObject {
    ...
    // The size of this object on the stack. 0 means a variable sized
    // object, ~0ULL means a dead object.
    uint64_t Size;
    ...
    /// and/or GC related) over a statepoint. We know that the address of
    /// the slot can't alias any LLVM IR value.  This is very similar to a
    /// Spill Slot, but is created by statepoint lowering is SelectionDAG,
    /// not the register allocator.
    bool isStatepointSpillSlot = false;

    /// Identifier for stack memory type analogous to address space. If this
    /// is non-0, the meaning is target defined. Offsets cannot be directly
    /// compared between objects with different stack IDs. The object may
    /// not necessarily reside in the same contiguous memory block as other
    /// stack objects. Objects with differing stack IDs should not be merged
    /// or replaced substituted for each other.
    //
    /// It is assumed a target uses consecutive, increasing stack IDs
    /// starting from 1.
    uint8_t StackID;
    ...
    // block and doesn't need additional handling for allocation beyond
    // that.
    bool PreAllocated = false;

    // If true, an LLVM IR value might point to this object.
    // Normally, spill slots and fixed-offset objects don't alias
    // IR-accessible objects, but there are exceptions (on PowerPC, for
    // example, some byval arguments have ABI-prescribed offsets).
    bool isAliased;
    ...
    StackObject(uint64_t Size, Align Alignment, int64_t SPOffset,
                bool IsImmutable, bool IsSpillSlot, const void *Alloca,
  ...
  /// allocations and the target must handle them as part of
  /// DYNAMIC_STACKALLOC lowering. All non-alloca stack objects have their
  /// alignment clamped to the base ABI stack alignment.
  ...
  /// above.  It then updates StackSize to contain the number of bytes that
  /// need to be allocated on entry to the function.
  ...
  /// This contains the size of the largest call frame if the target uses
  /// frame setup/destroy pseudo instructions (as defined in the
  /// TargetFrameInfo class).  This information is important for frame pointer
  /// elimination. It is only valid during and after prolog/epilog code
  /// insertion.
  ...
  CgBasicBlock *Save = nullptr;
  /// Not null, if shrink-wrapping found a better place for the epilogue.
  CgBasicBlock *Restore = nullptr;

  /// Size of the UnsafeStack Frame
  uint64_t UnsafeStackSize = 0;

public:
  explicit CgFrameInfo(Align StackAlignment, bool StackRealignable = false,
                       bool ForcedRealign = false)
      : StackAlignment(StackAlignment), StackRealignable(StackRealignable),
        ForcedRealign(ForcedRealign) {}

  CgFrameInfo(const CgFrameInfo &) = delete;
  ...
  /// CgFunction::shouldSplitStack() returns true.
  //
  ...
  // non-existent prologue, it would fail with an error. Mark the object file
  // so that such failures are not errors. See this Go language bug-report
  ...
  const void *getObjectAllocation(int ObjectIdx) const {
    assert(unsigned(ObjectIdx + NumFixedObjects) < Objects.size() &&
           "Invalid Object Idx!");
  ...
  uint64_t estimateStackSize(const CgFunction &MF) const;
  ...
  /// Returns true if the function contains operations which will lower down
  /// to instructions which manipulate the stack pointer.
  ...
  void computeMaxCallFrameSize(const CgFunction &MF);
  ...
  /// the prologue. Only used for debug info.
  ...
  int CreateStackObject(uint64_t Size, Align Alignment, bool isSpillSlot,
                        const void *Alloca = nullptr, uint8_t ID = 0);
  ...
  /// Notify the CgFrameInfo object that a variable sized object has been
  /// created.  This must be created whenever a variable sized object is
  /// created, whether or not the index returned is actually used.
  int CreateVariableSizedObject(Align Alignment, const void *Alloca);
  ...
  CgBasicBlock *getSavePoint() const { return Save; }
  void setSavePoint(CgBasicBlock *NewSave) { Save = NewSave; }
  CgBasicBlock *getRestorePoint() const { return Restore; }
  void setRestorePoint(CgBasicBlock *NewRestore) { Restore = NewRestore; }
  ...
  /// but that must be preserved - they are callee saved registers that are
  /// not saved.
  ///
  /// Before the PrologueEpilogueInserter has placed the CSR spill code, this
  /// method always returns an empty set.
  BitVector getPristineRegs(const CgFunction &MF) const;

  /// Used by the CgFunction printer to print information about
  /// stack objects. Implemented in CgFunction.cpp.
  void print(const CgFunction &MF, raw_ostream &OS) const;

  /// dump - Print the function to stderr.
  void dump(const CgFunction &MF) const;
};

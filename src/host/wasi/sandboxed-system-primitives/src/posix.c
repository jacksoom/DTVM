...
bool fd_prestats_init(struct fd_prestats *pt) {
...
static bool fd_prestats_grow(struct fd_prestats *pt, size_t min, size_t incr)
  ...
  if (pt->size <= min || pt->size < (pt->used + incr) * 2) {
    ...
    struct fd_prestat *prestats = malloc((uint32_t)(sizeof(*prestats) * size));
    if (prestats == NULL)
      return false;

    if (pt->prestats && pt->size > 0) {
      // memcpy(prestats, (uint32_t)(sizeof(*prestats) * size),
      //            pt->prestats, (uint32_t)(sizeof(*prestats) *
      //            pt->size));
      memcpy(prestats, pt->prestats, (uint32_t)(sizeof(*prestats) * pt->size));
    }
...
bool fd_prestats_insert(struct fd_prestats *pt, const char *dir,
                        __wasi_fd_t fd) {
...
static __wasi_errno_t fd_prestats_get_entry(struct fd_prestats *pt,
                                            __wasi_fd_t fd,
...
bool fd_table_init(struct fd_table *ft) {
...
static __wasi_errno_t fd_table_get_entry(struct fd_table *ft, __wasi_fd_t fd,
                                         __wasi_rights_t rights_base,
                                         __wasi_rights_t rights_inheriting,
                                         struct fd_entry **ret)
...
static bool fd_table_grow(struct fd_table *ft, size_t min, size_t incr)
  ...
  if (ft->size <= min || ft->size < (ft->used + incr) * 2) {
    ...
    // Grow the file descriptor table's allocation.
    struct fd_entry *entries = malloc((uint32_t)(sizeof(*entries) * size));
    if (entries == NULL)
      return false;

    if (ft->entries && ft->size > 0) {
      // memcpy(entries, (uint32_t)(sizeof(*entries) * size), ft->entries,
      //            (uint32_t)(sizeof(*entries) * ft->size));
      memcpy(entries, ft->entries, (uint32_t)(sizeof(*entries) * ft->size));
    }
...
// descriptor table.
static __wasi_errno_t fd_table_insert(struct fd_table *ft, struct fd_object *fo,
                                      __wasi_rights_t rights_base,
                                      __wasi_rights_t rights_inheriting,
                                      __wasi_fd_t *out)
...
__wasi_errno_t wasmtime_ssp_fd_prestat_dir_name(
  ...
  memcpy(path, prestat->dir, (uint32_t)path_len);
...
static __wasi_errno_t fd_object_get_locked(struct fd_object **fo,
                                           struct fd_table *ft, __wasi_fd_t fd,
                                           __wasi_rights_t rights_base,
                                           __wasi_rights_t rights_inheriting)
...
__wasi_errno_t wasmtime_ssp_fd_allocate(
  ...
  if (ret == 0 && (size_t)sb.st_size < offset + len)
...
static char *readlinkat_dup(int fd, const char *path, size_t *p_len) {
  char *buf = NULL;
  size_t len = 32;
  size_t len_org = len;

  for (;;) {
    char *newbuf = malloc((uint32_t)len);
    ...
    if (buf != NULL) {
      // bh_memcpy_s(newbuf, (uint32_t)len, buf, (uint32_t)len_org);
      memcpy(newbuf, buf, (uint32_t)len_org);
      free(buf);
    }
...
path_get(struct fd_table *curfds, struct path_access *pa, __wasi_fd_t fd,
  ...
  for (;;) {
    ...
    *file_end = '\0';
    ...
    if (strcmp(file, ".") == 0) {
      ...
      // expansion.
      if (ends_with_slashes || (flags & __WASI_LOOKUP_SYMLINK_FOLLOW) != 0) {
        symlink = readlinkat_dup(fds[curfd], file, &symlink_len);
        if (symlink != NULL)
          goto push_symlink;
  ...
  push_symlink:
    ...
    if (ends_with_slashes) {
      // bh_strcat_s(symlink, (uint32_t)symlink_len, "/");
      if (symlink_len > strlen(symlink)) {
        strncat(symlink, "/", symlink_len - strlen(symlink) - 1);
      }
    }
...
static bool validate_path(const char *path, struct fd_prestats *pt) {
  size_t i;
...
static void fd_readdir_put(void *buf, size_t bufsize, size_t *bufused,
                           const void *elem, size_t elemsize) {
  size_t bufavail = bufsize - *bufused;
  if (elemsize > bufavail)
    elemsize = bufavail;
  // bh_memcpy_s((char *)buf + *bufused, (uint32_t)bufavail, elem,
  //            (uint32_t)elemsize);
  memcpy((char *)buf + *bufused, elem, (uint32_t)elemsize);
  *bufused += elemsize;
}
...
static void convert_stat(const struct stat *in, __wasi_filestat_t *out) {
  *out = (__wasi_filestat_t){
      ...
      .st_atim = convert_timespec(&in->stat_st_atim),
      .st_mtim = convert_timespec(&in->stat_st_mtim),
      .st_ctim = convert_timespec(&in->stat_st_ctim),
  };
}
...
static void convert_utimens_arguments(__wasi_timestamp_t st_atim,
...
__wasi_errno_t wasmtime_ssp_poll_oneoff(
  ...
  if (nsubscriptions == 1 && in[0].u.type == __WASI_EVENTTYPE_CLOCK) {
    ...
    case __WASI_CLOCK_MONOTONIC:
      if ((in[0].u.u.clock.flags & __WASI_SUBSCRIPTION_CLOCK_ABSTIME) != 0) {
        // TODO(ed): Implement.
  ...
  struct fd_object **fos = malloc((uint32_t)(nsubscriptions * sizeof(*fos)));
  if (fos == NULL)
    return __WASI_ENOMEM;
  struct pollfd *pfds = malloc((uint32_t)(nsubscriptions * sizeof(*pfds)));
  if (pfds == NULL) {
    free(fos);
    return __WASI_ENOMEM;
  }
    ...
    case __WASI_EVENTTYPE_FD_WRITE: {
      ...
      if (error == 0) {
        ...
        pfds[i] = (struct pollfd){
            .fd = fd_number(fos[i]),
            .events =
                s->u.type == __WASI_EVENTTYPE_FD_READ ? POLLRDNORM : POLLWRNORM,
        };
      } else {
...
__wasi_errno_t wasmtime_ssp_args_get(
  ...
  // bh_memcpy_s(argv_buf, (uint32_t)argv_environ->argv_buf_size,
  //            argv_environ->argv_buf,
  //            (uint32_t)argv_environ->argv_buf_size);
  memcpy(argv_buf, argv_environ->argv_buf,
         (uint32_t)argv_environ->argv_buf_size);
...
__wasi_errno_t wasmtime_ssp_environ_get(
  ...
  for (size_t i = 0; i < argv_environ->environ_count; ++i) {
    environ[i] = environ_buf +
                 (argv_environ->environ_list[i] - argv_environ->environ_buf);
  }
  environ[argv_environ->environ_count] = NULL;
  // bh_memcpy_s(environ_buf, (uint32_t)argv_environ->environ_buf_size,
  //            argv_environ->environ_buf,
  //            (uint32_t)argv_environ->environ_buf_size);
  memcpy(environ_buf, argv_environ->environ_buf,
         (uint32_t)argv_environ->environ_buf_size);
  return __WASI_ESUCCESS;
}

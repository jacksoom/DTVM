...
static inline bool mutex_init(struct mutex *lock) REQUIRES_UNLOCKED(*lock) {
...
static inline bool rwlock_init(struct rwlock *lock) REQUIRES_UNLOCKED(*lock) {
...
static inline void rwlock_destroy(struct rwlock *lock)
    UNLOCKS(*lock) NO_LOCK_ANALYSIS {
...
static inline bool cond_init_monotonic(struct cond *cond) {
  bool ret = false;
#if CONFIG_HAS_PTHREAD_CONDATTR_SETCLOCK
...
static inline bool cond_init_realtime(struct cond *cond) {
  if (pthread_cond_init(&cond->object, NULL) != 0)
    return false;
#if !CONFIG_HAS_PTHREAD_CONDATTR_SETCLOCK ||                                   \
    !CONFIG_HAS_PTHREAD_COND_TIMEDWAIT_RELATIVE_NP
...
static inline bool cond_timedwait(struct cond *cond, struct mutex *lock,
  ...
  if (abstime) {
    ...
    ret = pthread_cond_timedwait_relative_np(&cond->object, &lock->object, &ts);

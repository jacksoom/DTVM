...
static inline void refcount_init(struct refcount *r, unsigned int count)
    PRODUCES(*r) {
...
/* Increment the reference counter. */
static inline void refcount_acquire(struct refcount *r) {
...
/* Decrement the reference counter, returning whether the reference
   dropped to zero. */
static inline bool refcount_release(struct refcount *r) {
  int old;
  sgx_spin_lock(&r->lock);
  old = (int)r->count;
  r->count--;
